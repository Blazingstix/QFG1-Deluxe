...
(procedure (localproc_0012 seconds &tmp [temp0 4] [temp4 400])
	(cls)
	(Format @temp4 &rest)
...
(procedure (InitHut)
	;if the hut hasn't flown away, then draw the hut, and the gate and BoneHead
	(if (Btst BABAYAGA_HUT_OPEN)
		;the hut's doors are open
		(= hutX 167)
		(= hutY 96)
		(hut
			setLoop: 4
		...
		;gate isn't drawn, neither is bonehead. but the mound of dirt is visible
	else
		;the hut's doors are closed, and the hut is standing tall
		(= hutX 143)
		(= hutY 40)
		(frontLeg
			setLoop: 0
			setCel: 0 ;CI: added to help with Retry
			setPri: 3
			ignoreActors:
			posn: (- hutX 20) hutY
			init:
		...
		(hut
			...
			init:
			stopUpd:
			;setScript: NULL ;CI: added to help with Retry
		)
		(if (not gateOpen)
			;set the gate closed, and hide the dirt mound, and set BoneHead
			...
			(if (< babaYagaHutStatus hutGAVEGEM) (eyes stopUpd: hide:))
		)
	)
)
...
(instance rm22 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vBabaHut vBonehead (GetEgoViewNumber vEgoFrogTransform) vTeleportGreen (GetEgoViewNumber vEgoDefeatedMagic))
		(LoadMany RES_SOUND 23 (GetSongNumber 28) (GetSongNumber 66))
		(super init:)
		...
		(if (== prevRoomNum 21)
			(cond 
				((Btst DIE_RETRY_INPROGRESS)
					;coming from a failed attempt to use the magic mirror against Baba Yaga
					(Bclr DIE_RETRY_INPROGRESS)
					;also... i dunno...
					(ego posn: 230 180 init: loop: loopN)
					(StopEgo)
					(HandsOn)
				)
				((< babaYagaStatus babaBRING)
					(ego view: (GetEgoViewNumber vEgoFrogTransform) loop: 0 cel: 7 posn: 152 160 ignoreActors:)
	...
	(method (handleEvent pEvent &tmp [temp0 50])
		(cond 
			...
			((EventEqualsString pEvent {hut of brown})
				(cond 
					...
					(
					(StringEquals (User inputLineAddr?) {now sit down})
						;CI: moved the points from here to the end of the hut walking.
						(if (not (hut script?))
							(hut setScript: walkEm)
						else
							(Print 22 7 #title {HUT} #at -1 20)
					...
					((Said 'look>')
						(cond 
							...
							((Said '/skull,head')
								...
								(if (not gateOpen)
									(if (< babaYagaHutStatus babaFINALE)
										(HighPrint 22 12)
										;The large skull on the gate seems to stare vacantly at you.
									else
										(HighPrint 22 16)
		...
		(switch (pEvent type?)
			(evSAID
				...
				(cond 
					...
					((Said 'open[<gate]')
						(if (== babaYagaHutStatus hutGAVEGEM)
							(HighPrint 22 38)
							;"Right!"
							(gate setScript: openGate)
						else
						...
						(cond 
							...
							((Said '//deal')
								(switch babaYagaHutStatus
									...
									(hutMETSKULL (localproc_006d))
									(hutNODEAL (localproc_0088))
									(hutDEALMADE (localproc_0088))
									(hutGAVEGEM
										(HighPrint 22 42)
...
(instance putEyes of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(HighPrint 22 57 25 4) ;What are the two extra tuples for?
				;You place the glowing gem inside the skull.
				(ego use: iMagicGem)
				(eyes startUpd: show:)
				(= babaYagaHutStatus hutGAVEGEM)
				(self cue:)
			)
			(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(11
				...
				(if crushedByHut
					(ego hide:)
					;(ego dispose:) ;CI: NOTE: replaced dispose with hide, because we're going to implement a retry instead.
				)
				(= cycles 1)
			)
			(12
			...
			(14
				...
				(hut setScript: NULL)
				(if crushedByHut
					(EgoDead DIE_RETRY DIE_BABAYAGA_CRUSHED 22 69
						#title { Bad strategy!_}
						#icon (GetEgoViewNumber vEgoDefeatedMagic) 3 7
						;You really brought down the house!  Too bad you were under it at the time.
					)
					;CI: NOTE: moving ego's position, and resetting the hut back to standing.
					(StopEgo)
					(ego show: posn: 230 180 loop: loopN)
					(= crushedByHut FALSE)
					(Bclr BABAYAGA_HUT_OPEN)
					(hutDoor dispose:)
					(InitHut)
					...
					;CI: Moved score here. Only hit if the hut sits down and not on the hero.
...
(instance flyAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(tpSound number: (GetSongNumber 28) init: play:)
				(TP setCycle: EndLoop)
...
(instance frogIn of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(TP setCycle: CycleTo 6 cdFORWARD self)
			)
			(1
				(tpSound number: (GetSongNumber 28) init: play:)
				(ego init:)
			...
			(5
				...
				(switch babaYagaStatus
					(babaFETCH
						...
						(Bset BABAYAGA_CURSE)
						(= dayCursedByBabaYaga currentDay)
					)
