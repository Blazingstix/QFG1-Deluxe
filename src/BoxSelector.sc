...
(instance nameEdit of DEdit
	...
	(method (handleEvent pEvent &tmp temp0)
		...
		(if (& state dSelected)
			(if (== direction (pEvent type?))
				(pEvent type: keyDown)
				(switch (pEvent message?)
					(dirS
						(pEvent message: DOWNARROW)
					)
					(dirN 
						(pEvent message: UPARROW)
					)
				)
			)
			(if
			(and (& state dSelected) (== keyDown (pEvent type?)))
				(switch (pEvent message?)
					(DOWNARROW
						(pEvent message: TAB)
					)
					(UPARROW 
						(pEvent message: SHIFTTAB)
					)
				)
			)
		)
...
(class BoxSelector of DItem
	...
	(method (handleEvent pEvent &tmp temp0)
		(if (pEvent claimed?) (return 0))
		(if (= temp0 (super handleEvent: pEvent))
			(return temp0)
		)
		(if (not (& state dSelected)) (return 0))
		(if (== direction (pEvent type?))
			(pEvent type: keyDown)
			(switch (pEvent message?)
				(dirS
				...
				(dirE
					(pEvent message: RIGHTARROW)
				)
				(dirW
					(pEvent message: LEFTARROW)
				)
				(else  
	...
	(method (draw)
		;draw a red selected box, or a white "not-selected" box
		(DrawCel vCharSheet 1
	...
	(method (incr amount)
		...
		(if (< currentPool amount) (= amount currentPool))
		(if (== [egoStats value] 0)
			(if (< currentPool 15) (return))
			(= [egoStats value] 5)
			(= currentPool (- currentPool 15))
		else
			(= [egoStats value] (+ [egoStats value] amount))
			(= currentPool (- currentPool amount))
		)
		(self select: 0)
	...
	(method (dec amount &tmp temp0)
		(if
			(not
				(= temp0 (- [egoStats value] [lastViewedSkills value]))
			)
...
(instance chAlloc of Room
	...
	(method (init &tmp cntrl doLoop nextRoom)
		(Load RES_SCRIPT 204)
		(Load RES_SCRIPT 205)
		(Load RES_VIEW (GetEgoViewNumber vEgoCharSheet))
		(Load RES_VIEW vCharSheet)
		...
		;CI: not sure if this is correct or not, based on ASM
		...
		(statDial 
			window: systemWindow 
			add: nameEdit strengthSel intSel agilSel 
				 vitSel luckSel wpnSel parrySel dodgeSel 
				 stealthSel pickSel throwSel climbSel 
				 magicSel exitButton canButton 
			open: (| stdWindow NOSAVE) -1
			;open: vBLACK vWHITE ;CI: this was incorrect
		)
		
		((ScriptID 204 0) useWindow: FALSE showBars: FALSE init:)
		(Display 203 1 p_at 62 109 p_mode 0 p_font 300 p_color statColourNew)
		;Points Available
		(poolBar max: maxPool value: currentPool init:)
		(Display 203 2 p_at 165 127 p_width 150 p_mode 0 p_font 300 p_color statColourNew)
...
; 			pushi    p_color
; 			lsg      statColourNew
; 			callk    Display,  22	;(Display 203 1 p_at 62 109 p_mode 0 p_font 300 p_color statColourNew)
; 										;Points Available
; 			pushi    #max
; 			pushi    1
; 			lsl      maxPool
...
; 			pushi    p_color
; 			lsg      statColourNew
; 			callk    Display,  26	;(Display 203 2 p_at 165 127 p_width 150 p_mode 0 p_font 300 p_color statColourNew)
; 										;TAB to move around,\nArrows to adjust values.
