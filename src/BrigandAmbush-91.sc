...
(instance rm91 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vArchers vArrows (GetEgoViewNumber vEgoJumping))
		...
		(StopEgo)
		(cSound stop:)
		(brigandS number: (GetSongNumber 73) init: play:)
	...
	(method (doit &tmp egoLoop egoPriority tmpEgoX tmpEgoY)
		(= egoLoop (ego loop?))
		...
		(while (<= local147 24)
			(if (!= [newView local147] 0)
				...
				([newView (+ local147 3)]
					...
					posn: (+ tmpEgoX local146) (+ tmpEgoY [newView local147])
				)
			)
			(= local147 (+ local147 4))
		)
			...
			(mouseDown
				(cond 
					...
					((MouseClaimed ego event emRIGHT_BUTTON)
						(HighPrint (Format @temp0 91 0 @userName)
	...
	(method (handleEvent event)
		(if
			(or
				...
				(MouseClaimed archer2 event emRIGHT_BUTTON)
				(MouseClaimed archer4 event emRIGHT_BUTTON)
				(MouseClaimed archer5 event emRIGHT_BUTTON)
				(MouseClaimed archer6 event emRIGHT_BUTTON)
			)
			(HighPrint 91 13)
			;A brigand archer!
		)
...
(instance archer3 of Prop
	...
	(method (handleEvent event)
		(if (MouseClaimed archer3 event emRIGHT_BUTTON)
			(HighPrint 91 14)
			;One bad dude!
		)
	)
...
(instance spearman1 of View
	...
	(method (handleEvent event)
		(if
			(or
				(MouseClaimed spearman1 event emRIGHT_BUTTON)
				(MouseClaimed spearman2 event emRIGHT_BUTTON)
				(MouseClaimed spearman3 event emRIGHT_BUTTON)
				(MouseClaimed spearman4 event emRIGHT_BUTTON)
				(MouseClaimed spearman5 event emRIGHT_BUTTON)
			)
			(HighPrint 91 15)
...
(class Shooter of Script
	...
	(method (changeState newState &tmp tmpEgoX)
		(switch (= state newState)
			(0
				...
				(switch register
					...
					(4
						...
						(= archRestCel 3)
						(cond 
							((< 194 tmpEgoX) (client setLoop: 3) (= arrowLoop 3))
							((< 183 tmpEgoX) (client setLoop: 4) (= arrowLoop 3))
			...
			(1
				...
				(client setCycle: CycleTo (- (NumCels client) 2) cdFORWARD self)
			)
			(2
				((= arrowID (Prop new:))
					view: vArrows
			...
			(5
				(client setCycle: CycleTo archRestCel cdFORWARD)
				(arrowID
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			(0
				...
				(if (or (<= [egoStats HEALTH] 0) egoHeadShot)
					(= cycles 3)
				else
					(self dispose:)
				)
			...
			(1
				...
				(EgoDead DIE_RETRY DIE_BRIGAND_AMBUSH 91 17
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(if local149
					(ego
						view: (GetEgoViewNumber vEgoJumping)
						setLoop: loopE
						setCel: 0
						illegalBits: 0
						ignoreActors:
