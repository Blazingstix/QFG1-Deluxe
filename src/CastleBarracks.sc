...
(instance rm38 of Room
	...
	(method (init &tmp tmpEgoY)
		...
		(ego
			posn: 319 tmpEgoY
			init:
			setMotion: MoveTo 305 (if (< tmpEgoY 115) 115 else tmpEgoY)
		)
...
(instance guardHead of Actor
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed guardBody event emRIGHT_BUTTON)
					(HighPrint 38 9)
			...
			(evSAID
				(cond 
					...
					((Said 'kill/guard,man,gatekeeper')
						(EgoDead DIE_RETRY DIE_CASTLEBARRACKS 38 13
							#title {OH NOOOOOOO!}
...
(instance onBarracks of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBarracks event emRIGHT_BUTTON)
				(HighPrint 38 2)
				;The building's appearance is deceptive; it evidently extends well into the castle wall.
				;From your present vantage point, the barracks looks like it could house some sixty guards.
				)
...
(instance onFence of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onFence event emRIGHT_BUTTON)
				(HighPrint 38 18)
				;Just a fence.
				)
		)
...
(instance onMountains of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onMountains event emRIGHT_BUTTON)
				(HighPrint 38 19)
...
(instance onTrees of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onTrees event emRIGHT_BUTTON)
				(HighPrint 38 20)
...
(instance onWall1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onWall1 event emRIGHT_BUTTON)
					(MouseClaimed onWall2 event emRIGHT_BUTTON)
					(MouseClaimed onWall3 event emRIGHT_BUTTON)
				)
				(HighPrint 38 21)
