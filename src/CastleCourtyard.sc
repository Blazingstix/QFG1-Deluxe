...
(instance rm39 of Room
	...
	(method (init &tmp tmpEgoY)
		(Load RES_VIEW vCastleGuards)
		(if
			(and
				(< TIME_DAWN timeZone)
				(< timeZone TIME_SUNSET)
				(not (Btst DEFEATED_WEAPON_MASTER))
				(!= dayLastFoughtWeaponMaster currentDay)
			)
			(LoadMany RES_SCRIPT SKILLED 221 222 223 224 220 218 217 216)
			(LoadMany RES_VIEW vWeaponMasterTalk vWeaponMaster (GetEgoViewNumber vEgoFightWithSword) (GetEgoViewNumber vEgoDefeated) (GetEgoViewNumber vEgoSwordDefeated))
		)
	...
	(method (dispose)
		(if (>= currentTime 3000) (Bclr FLAG_239))
		(super dispose:)
	)
