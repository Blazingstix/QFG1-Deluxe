...
(instance rm37 of Room
	...
	(method (init)
		(super init:)
		(if isNightTime (Load RES_VIEW (GetEgoViewNumber vEgoDefeated)) (Load RES_VIEW (GetEgoViewNumber vEgoClimbing)))
		(Load RES_VIEW vCastleGate)
		(Load RES_SOUND (GetSongNumber 90))
		(lBush init: addToPic:)
		(rBush init: addToPic:)
		(portSound number: (GetSongNumber 90) init:)
	...
	(method (doit)
		...
		(cond 
			((and (Btst SAVED_ELSA) gateOpen (< (ego y?) 163)) (curRoom newRoom: 600))
			...
			(evSAID
				(cond 
					...
					((Said 'climb,climb[/gate,wall,gatehouse]')
						(if isNightTime
							(cond 
								;you need 10 or higher climbing to climb the castle
								((< [egoStats CLIMB] tryClimbCastleGateNoSkill)
									(HighPrint 37 2)
									;Leave the climbing to those who know how.
								)
								;if you have more than 35 climbing skill, you can climb the castle gate.
								((> [egoStats CLIMB] tryClimbCastleGate)
									(if
										(or
											(and
												(<= 78 (ego x?) 114)
								...
								;if we have between 10 and 35 climbing, then we check if we're in the right place. If so, climb then fall.
								(
									(or
										(and
											(<= 78 (ego x?) 114)
									...
									(= fellOffWall (+ fellOffWall 1))
									(self setScript: egoClimbsWall)
								)
								;finally, we have enough to try, but not enough to succeed, and we're in the wrong place, 
...
(instance closeGate of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(if crushedByGate
					(EgoDead DIE_RETRY DIE_CASTLE_GATE 37 18
						#title {OH NOOOOOOO!}
						#icon (GetEgoViewNumber vEgoDeathScenes) 0 0 ;EO - "known" should be "know". ;CI: fixed.
		...
		(switch (= state newState)
			...
			(5
				(if (not (TakeDamage 10))
					(EgoDead DIE_RETRY DIE_CASTLE_FALL 37 20
					...
					(ego setHeroRetry: posn: 161 187 setLoop: loopN)
					(client setScript: NULL)
					(HandsOn)
					(StopEgo)
				else
...
(instance guard of Actor
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed guard event emRIGHT_BUTTON)
					(HighPrint 37 28)
...
(instance onCastleGate of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onCastleGate event emRIGHT_BUTTON)
				(cond 
					(isNightTime
						(HighPrint 37 81)
...
(instance onCastle of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onCastle event emRIGHT_BUTTON)
				(HighPrint 37 84)
...
(instance onLGateHouse of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onLGateHouse event emRIGHT_BUTTON)
					(MouseClaimed onRGateHouse event emRIGHT_BUTTON)
				)
				(cond 
					(isNightTime
						(HighPrint 37 85)
...
(instance onMountain of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onMountain event emRIGHT_BUTTON)
					(MouseClaimed onM1 event emRIGHT_BUTTON)
					(MouseClaimed onM2 event emRIGHT_BUTTON)
					(MouseClaimed onM3 event emRIGHT_BUTTON)
				)
				(HighPrint 37 87)
