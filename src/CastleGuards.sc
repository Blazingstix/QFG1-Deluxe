...
(instance rm41 of Room
	...
	(method (init &tmp tmpEgoX tmpEgoY)
		(Load RES_VIEW vCastleGuards2)
		(cSound fade:)
		(super init: &rest)
		(StatusLine enable:)
		...
		(switch prevRoomNum
			(38
				(if (and (Btst SAVED_BARNARD) (not (Btst OBTAINED_BARNARD_REWARD)))
					...
					(if (< tmpEgoY 110) (= tmpEgoY 110))
					(ego posn: 1 tmpEgoY setMotion: MoveTo 15 tmpEgoY)
				)
			)
			(39
				(if (and (Btst SAVED_BARNARD) (not (Btst OBTAINED_BARNARD_REWARD)))
					...
					(ego posn: tmpEgoX 189)
					(cond 
						((< tmpEgoX 10) (ego setMotion: MoveTo (+ tmpEgoX 25) 175))
						((> tmpEgoX 310) (ego setMotion: MoveTo (- tmpEgoX 25) 175))
						(else (ego setMotion: MoveTo tmpEgoX 175))
					)
				)
			)
			(40
				(if (and (Btst SAVED_BARNARD) (not (Btst OBTAINED_BARNARD_REWARD)))
					...
					(if (< tmpEgoY 110) (= tmpEgoY 110))
					(ego posn: 319 tmpEgoY setMotion: MoveTo 305 tmpEgoY)
				)
			)
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed ego event emRIGHT_BUTTON)
					(HighPrint 41 0)
			...
			(evSAID
				(cond 
					...
					((or (Said 'nap') (Said 'go[<to]/nap'))
						...
						(if (and (< 750 currentTime) (< currentTime 2550))
							(FixTime 21)
						)
						(curRoom newRoom: 37)
					)
...
(instance lGuard of Prop
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed lGuard event emRIGHT_BUTTON)
					(HighPrint 41 13)
			...
			(evSAID
				(cond 
					...
					((Said 'fight,kill/guard,man,gatekeeper')
						(EgoDead DIE_RETRY DIE_CASTLEGUARDS 41 15
							#title {Getting into the castle the hard way.}
							#icon (GetEgoViewNumber vEgoJailDeath) 3 0
...
(instance rGuard of Prop
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed rGuard event emRIGHT_BUTTON)
					(HighPrint 41 20)
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if
					(or
						(and local0 (MouseClaimed door1 event emRIGHT_BUTTON))
						(and local0 (MouseClaimed door2 event emRIGHT_BUTTON))
						(and
							(MouseClaimed door1 event emRIGHT_BUTTON)
							(not (MouseClaimed torch event emRIGHT_BUTTON))
						)
						(and
							(MouseClaimed door2 event emRIGHT_BUTTON)
							(not (MouseClaimed torch event emRIGHT_BUTTON))
						)
					)
					(HighPrint 41 21)
...
(instance torch of Prop
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if
				(and (not local0) (MouseClaimed torch event emRIGHT_BUTTON))
					(HighPrint 41 22)
...
(instance onTrees1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onTrees1 event emRIGHT_BUTTON)
					(MouseClaimed onTrees2 event emRIGHT_BUTTON)
				)
				(HighPrint 41 23)
			)
		)
...
(instance onWeed1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(not (MouseClaimed ego event emRIGHT_BUTTON))
					(or
...
(instance onStones of RFeature
	...
	;CI: NOTE: This is a manual decompilation from asm code. The original asm is below.
	(method (handleEvent event)
		(cond
			((super handleEvent: event))
...
;			pushi    1
;			lsp      event
;			super    RFeature,  6		;((super handleEvent: event) )
;			bnt      code_0eca		;if not true, 
;			jmp      code_0fa3		;else go to end
;code_0eca:
;			pushi    3
;			lofsa    onStones
;			push    
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onStones event emRIGHT_BUTTON)
;			bnt      code_0f9b				;if false, then go to "else" condition
;			
;			pushi    3
;			lsg      ego
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed ego event emRIGHT_BUTTON)
;			bt       code_0f35				;if true go to first print.
;			
;			pushi    3
;			lofsa    onWeed1
;			push    
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onWeed1 event emRIGHT_BUTTON)
;			bt       code_0f35
;
;			pushi    3
;			lofsa    onWeed2
...
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onWeed2 event emRIGHT_BUTTON)
;			bt       code_0f35
;
;			pushi    3
;			lofsa    onWeed3
...
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onWeed3 event emRIGHT_BUTTON)
;			bt       code_0f35
;
;			pushi    3
;			lofsa    onWeed4
...
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onWeed4 event emRIGHT_BUTTON)
;			bt       code_0f35
;			
;			pushi    3
;			lofsa    onWeed5
...
;			lsp      event
;			pushi    emRIGHT_BUTTON
;			callb    MouseClaimed,  6	;((MouseClaimed onWeed5 event emRIGHT_BUTTON)
;
;code_0f35:		;first HighPrint command (where we actually do stuff)
;			not     						;flip the value.
;			bnt      code_0f9b				;if false, then go to "else" condition
;			
;			lsl      stoneLookCount
...
;			ldi      0
;			eq?     
;			bnt      code_0f4f		;if stoneLookCount <> 0 then goto next message
;			
;			pushi    2
;			pushi    41
;			pushi    26
;			callb    HighPrint,  4
;			;These flagstones are from a rock quarry in eastern Germany.
;			+al      stoneLookCount		;stoneLookCount++
;			jmp      code_0f97	;goto final clean-up before end
;
;code_0f4f:
;			dup     
;			ldi      1
;			eq?     
;			bnt      code_0f63		;if stoneLookCount <> 1 then goto next message
;			
;			pushi    2
;			pushi    41
;			pushi    27
...
;			ldi      2
;			eq?     
;			bnt      code_0f77		;if stoneLookCount <> 2 then goto next message
;			
;			pushi    2
;			pushi    41
;			pushi    28
...
;			ldi      3
;			eq?     
;			bnt      code_0f8b		;if stoneLookCount <> 3 then goto final message
;			
;			pushi    2
;			pushi    41
;			pushi    29
...
;			pushi    41
;			pushi    30
;			callb    HighPrint,  4		;(HighPrint 41 30)
;			;I don't know!
;			ldi      0
;			sal      stoneLookCount			;(= stoneLookCount 0)
;
;code_0f97:	;final clean-up
;			toss    			;drop all working variables
;			jmp      code_0fa3	;go to END
;code_0f9b:		;else (cond)
;			pushi    #claimed
;			pushi    1
;			pushi    0
;			lap      event			;(event claimed: TRUE)
;			send     6
;code_0fa3:		;end of (cond)
;			ret     
;		)
;	)
