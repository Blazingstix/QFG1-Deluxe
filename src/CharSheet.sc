...
(procedure (DisplayStatHeading x y &tmp fgColor)
	...
	(= fgColor (if (< colourCount 16) vBLACK else vBLUE))
...
(procedure (GetStatColor skill)
	...
	(return
		(if (== [egoStats skill] [lastViewedSkills skill])
			statColour
		else
			statColourNew
		)
...
(procedure (DisplayStat x y skill highlightNew &tmp fg [str 6])
	...
	(= fg (if highlightNew (GetStatColor skill) else statColour))
	(DisplayStatBar
		x y fg
...
(class CharSheet of Object
	...
	(method (init &tmp x)
		...
		(DrawCel vCharSheet 2 0 10 23 -1)
		;draw the hero
		(DrawCel (GetEgoViewNumber vEgoCharSheet) 0 heroType x 32 -1)
		(if (== heroType MAGE)
			;if it's a MAGE, then also draw the magic spiral.
			(DrawCel vCharSheet 1 0 40 52 -1)
		)
	...
	(method (update &tmp fgColor [temp1 20])
		...
		(= fgColor (if showBars (GetStatColor HEALTH) else statColour))
		(DisplayStatBar
			103 148 fgColor
			(Format @temp1 204 20
				(/ (+ [egoStats HEALTH] 1) 2)
		...
		(= fgColor (if showBars (GetStatColor STAMINA) else statColour))
		(DisplayStatBar
			103 160 fgColor
			(Format @temp1 204 20
		...
		(= fgColor (if showBars (GetStatColor MANA) else statColour))
		(DisplayStatBar
			103 172 fgColor
			(Format @temp1 204 20 
				[egoStats MANA] 
