...
(instance seed of Actor
	(properties
		view vSpittingSpirea ;the seed loop
	)
)
...
(instance rm76 of Room
	...
	(method (init)
		;(Printf {FreeHeap: %u\nLargestPrt: %u\nDiff: %u} (MemoryInfo FreeHeap) (MemoryInfo LargestPtr) (- (MemoryInfo FreeHeap) (MemoryInfo LargestPtr)))
		(if (not isNightTime)
			(LoadMany RES_VIEW vDryad (GetEgoViewNumber vEgoKilledByDryad))
			(if (or (Btst SMASHED_FLOWER1) (Btst SMASHED_FLOWER2) (Btst SMASHED_FLOWER3))
				(Load RES_VIEW vSpittingSpirea)
				(LoadMany RES_SOUND (GetSongNumber 18) (GetSongNumber 27))
			)
		else
			(Load RES_SCRIPT 7)
			(Load RES_VIEW (GetEgoViewNumber vEgoSleep))
		)
		(LoadMany RES_VIEW vDryadRoom (GetEgoViewNumber vEgoGetFaeryDust) (GetEgoViewNumber vEgoThrowing))
		(LoadMany RES_SOUND 97 98)
		(if (Btst STAG_PRESENT) (Load RES_VIEW vStag))
		(super init:)
		...
		(if (and (not isNightTime) (Btst STAG_PRESENT))
			(stag
				view: vStag
				x: 318
				y: 145
	...
	(method (doit)
		(cond 
			((and local4 (not (ego script?)) (< (stag x?) 50))
				(= local4 0)
				(HighPrint 76 7)
		...
		(cond 
			((and (== (ego edgeHit?) EAST) (>= dryadState 1))
				(Bclr STAG_PRESENT)
				(curRoom newRoom: 77)
			)
			;here's the conditions that will cause the Dryad to appear
			((and
					(< (ego x?) 200)
					(== dryadState 1)
					(not isNightTime)
				...
				(cond 
					...
					((Said 'cast>')
						(switch (= spell (SaidCast event))
							...
							(DAZZLE
								(if (CastSpell spell)
									(if (and (Btst STAG_PRESENT) (< (stag x?) 50)) (= local4 1))
									(if (== dryadState 2) (= local3 1))
									(AnimateDazzle)
								)
							)
							(FLAMEDART
					...
					((Said 'throw/dagger,dagger')
						(= temp1 (if (cast contains: stag) stag else NULL))
						(if (AnimateThrowingDagger temp1)
							(if (== dryadState 2) (= local3 1))
							(if (Btst STAG_PRESENT) (= local4 1) (Bset STAG_HURT))
							(if (cast contains: stag)
								(FaceObject ego stag)
								(RedrawCast)
							)
...
(instance outOfTree of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(dryad setLoop: 1 cel: 0 setCycle: CycleTo 2 cdFORWARD self)
			)
			(2
				(dryad
					moveSpeed: 1
			...
			(3
				(cond 
					((or castFlame (Btst STAG_HURT))
						(dryad setLoop: 6 cel: 6 forceUpd: setScript: egoToStag)
						;(dryad setLoop: 6 cel: 6 forceUpd: setScript: goToSleep)
						;CI: NOTE: this is testing if there's a problem
						;(curRoom newRoom: 77)
					)
					...
					((or (Btst SMASHED_FLOWER1) (Btst SMASHED_FLOWER2) (Btst SMASHED_FLOWER3))
						(dryad setLoop: 6 cel: 6 forceUpd: setScript: egoToPlant)
					)
					((Btst MET_DRYAD)
						(HighPrint 76 53)
...
(instance hasSeed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(ego view: (GetEgoViewNumber vEgoGetFaeryDust) setLoop: 0 cel: 1 forceUpd:)
				(= cycles 15)
			)
			(3
				(ego cycleSpeed: 1 setCycle: CycleTo 3 cdFORWARD)
				(= cycles 10)
			)
			(4
				(HighPrint 76 56)
				...
				(ego use: iSeed setCycle: EndLoop self)
			)
			(5
				(if (or (Btst SMASHED_FLOWER1) (Btst SMASHED_FLOWER2) (Btst SMASHED_FLOWER3))
					(HighPrint 76 57)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				...
				(bush ignoreActors: init: stopUpd: addToPic:)
				(ego view: (GetEgoViewNumber vEgoSleep) setLoop: 0 setCel: 0) ;sleeping view
				(= seconds 3)
			)
			(2
				(TimedPrint 5 76 81)
...
(instance oMyGod of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(dryad setCycle: CycleTo 3 cdFORWARD)
				(= seconds 3)
			)
...
(instance egoToPlant of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				;CI: NOTE: This script appears to cause the fragment problems
;;;				(stag setScript: stagBolts)
				(HighPrint 76 72)
			...
			(2
				(ego
					view: (GetEgoViewNumber vEgoKilledByDryad)
					setLoop: 0
					cel: 0
					cycleSpeed: 2
					setCycle: EndLoop self
			...
			(4
				(ego
					view: vSpittingSpirea ;the spitting spirea plant, animation
					setLoop: 1
					cel: 0
			...
			(11
				(EgoDead DIE_RETRY DIE_DRYAD_PLANT 76 74
					#title {Plant it, fella!}
					#icon (GetEgoViewNumber vEgoKilledByDryad) 2 6
				...
				;clear the smashed plants
				(client setScript: NULL)
				(seed dispose:)
				(dryad dispose:)
				(spitSound dispose:)
				(gulpSound dispose:)
				(stag setScript: NULL dispose:)
				;(Printf {FreeHeap: %u\nLargestPrt: %u\nDiff: %u\nLargestHandle: %u\nFreeHunk: %u\nTotalHunk: %u} (MemoryInfo FreeHeap) (MemoryInfo LargestPtr) (- (MemoryInfo FreeHeap) (MemoryInfo LargestPtr)) (MemoryInfo LargestHandle) (MemoryInfo FreeHunk) (MemoryInfo TotalHunk))
				(Bclr SMASHED_FLOWER1)
				(Bclr SMASHED_FLOWER2)
				(Bclr SMASHED_FLOWER3)
				;(self dispose:)
				(curRoom newRoom: 16) ;go to Spitting Spirea room.
			)
		)
	)
)
...
(instance egoToStag of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				;CI: NOTE: This script seems to cause the fragmentation errors.
;;;				(stag setScript: stagBolts)
				(cond 
					((Btst STAG_HURT)
						(HighPrint 76 75)
			...
			(4
				(EgoDead DIE_RETRY (if (Btst STAG_HURT) DIE_DRYAD_STAG else DIE_DRYAD_FIRE) 76 80
					#title {Oh, deer!}
					#icon (GetEgoViewNumber vEgoKilledByDryad) 1 5
