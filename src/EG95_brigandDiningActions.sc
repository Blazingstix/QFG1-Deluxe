...
(procedure (SaidEndGameBrigands evt &tmp spell)
	(cond 
		((Said 'look>')
			(cond 
		...
		((Said 'open[/gate,door]')
			(cond 
				((ego inRect: 34 75 73 95) (ego setScript: (ScriptID 95 26)))
				((ego inRect: 250 162 310 190)
					(if ((ScriptID 95 0) notify: 3)
						(HighPrint 160 11)
						;You don't dare; the brigands are coming.
		...
		((Said 'force,move,get,use/chair')
			(cond 
				...
				((ego inRect: 228 106 320 124) (ego setScript: (ScriptID 159 0)))
				(
					(or
		...
		(
			(Said
				'beat,force,shove,use,move,knock/candelabra,candle,candlestick'
			)
			(cond 
				...
				((ego inRect: 271 0 320 96) (ego setScript: (ScriptID 159 1)))
				(else
					(HighPrint 160 24)
