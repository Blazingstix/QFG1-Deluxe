...
(public
	...
	head 6
	door3 7
	door7 8 ;this is the door that will crush Ego is given the chance.
	door11 9
	door12 10
	door13 11
	behindDoor7 12	;this is the true door to the brigand leader's room
	behindDoor11 13
	chain 14
	arrow 15
	effect 16
...
(instance yorick of Prop
	...
	(method (handleEvent event)
		(cond 
			...
			((Said 'chat>')
				(cond 
					((Btst JESTER_NO_MORE_TALKING) (NoTalking) (event claimed: 1))
					((Said '/yorick')
...
(instance behindDoor7 of Prop
	;this is the true door to the brigand leader's room
	(properties
		y 96
		x 95
		view vJesterRoom
...
(instance door7 of Prop
	;this is the door that will crush Ego is given the chance.
	(properties
		y 95
		x 93
		view vJesterRoom
...
(instance rm96 of Room
	...
	(method (resetRoom)
		...
		(Bclr FLAG_264)
		(Bclr FLAG_265)
		(Bclr JESTER_NO_MORE_TALKING)
		(Bclr BEFRIENDED_JESTER)
		(Bclr FLAG_269)
		(Bclr FLAG_270)
		(Bclr FLAG_271)
		;TODO: remove the 2 solved puzzles the hero can do in this room.
		(= curRoomNum 95)
		(curRoom newRoom: 96)
	)
	
	(method (init)
		(LoadMany RES_VIEW vYorick (GetEgoViewNumber vEgoGrab) vJesterRoom (GetEgoViewNumber vEgoFlattenedByJesterDoor) vJesterTrapDoor (GetEgoViewNumber vEgoFall2) (GetEgoViewNumber vEgoShock) (GetEgoViewNumber vEgoClimbing) (GetEgoViewNumber vEgoFallDown) (GetEgoViewNumber vEgoDefeatedMagic)) ;CI: Also add vYorickLeave when they get split up
		(LoadMany RES_SCRIPT REVERSE JUMP CHASE WANDER 225 226 227 228 229 230 231 232 233 234)
		(LoadMany RES_SOUND
			82
			(GetSongNumber 86)
			(GetSongNumber 85)
		...
		(if (and (not (Btst FLAG_263)) (not (Btst OPENING_LEADER_DOOR)))
			(cond 
				...
				((and (== (ego edgeHit?) WEST) (< (ego y?) 150))
					(Bset FLAG_263)
					(if (< (ego y?) 100)
						(ego setPri: RELEASE setScript: (ScriptID 228 3))
					else
						(if local6 (= local5 1))
		...
		(cond 
			...
			(
				(or
					...
					(MouseClaimed globe event emRIGHT_BUTTON)
				)
				(HighPrint 96 51)
			...
			(
				(or
					...
					(and
						(MouseClaimed head event emRIGHT_BUTTON)
						(not (MouseClaimed arrow event emRIGHT_BUTTON))
					)
					(MouseClaimed yorick event emRIGHT_BUTTON)
				)
				(HighPrint 96 52)
				;It's "ME".
			)
			(
				(or
					(MouseClaimed arrow event emRIGHT_BUTTON)
					(MouseClaimed MESign event emRIGHT_BUTTON)
				)
				(HighPrint 96 53)
			...
			((MouseClaimed Door6 event emRIGHT_BUTTON)
				(HighPrint 96 55)
			...
			((MouseClaimed doorSign event emRIGHT_BUTTON)
				(HighPrint 96 57)
				;The arrow points to a door.  Who could've guessed?
				)
			((MouseClaimed secretSign event emRIGHT_BUTTON)
				(HighPrint 96 58)
			...
			(
				(or
					...
					(MouseClaimed exitSign event emRIGHT_BUTTON)
				)
				(HighPrint 96 59)
				;A sign points to a door.  The egress?
			)
			((MouseClaimed door11 event emRIGHT_BUTTON)
				(HighPrint 96 60)
				;It's another door!
				)
			((MouseClaimed door7 event emRIGHT_BUTTON)
				(HighPrint 96 61)
			...
			(
				(or
					(Said 'look/button')
					(MouseClaimed littleButton event emRIGHT_BUTTON)
				)
				(HighPrint 96 62)
			...
			(
				(or
					(Said 'look/box')
					(MouseClaimed door3 event emRIGHT_BUTTON)
				)
				(HighPrint 96 63)
				;It's a box of some sort...or so the sign says.  There is a button next to the box.
			)
			((MouseClaimed door12 event emRIGHT_BUTTON)
				(HighPrint 96 64)
				;A pet door?  It's shorter than you are.
				)
			((MouseClaimed littleSign event emRIGHT_BUTTON)
				(if
			...
			(
				(or
					(Said 'look/ladder')
					(MouseClaimed trap4 event emRIGHT_BUTTON)
				)
				(HighPrint 96 66)
			...
			(
				(or
					...
					(MouseClaimed door13 event emRIGHT_BUTTON)
				)
				(HighPrint 96 67)
				;Have a nice day!
			)
			(
				(or
					(Said 'look/dot[<polka]')
					(MouseClaimed polkaDots event emRIGHT_BUTTON)
				)
				(HighPrint 96 68)
		...
		(switch (event type?)
			(evSAID
				(cond 
					...
					((Said 'nap')
						(EgoDead DIE_RETRY DIE_ENDGAME_NAP 96 79
					...
					((Said 'open,unbar,unlock[/door]')
						(cond 
							...
							((== (ego onControl: origin) cLRED)
								(if (Btst FLAG_264)
									(ego setScript: (ScriptID 232 1)) ;finalExit
								else
									(User canInput: FALSE)
