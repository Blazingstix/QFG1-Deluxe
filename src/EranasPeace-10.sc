...
(instance rm10 of Room
	...
	(method (init)
		(Load RES_VIEW vEranasPeace (GetEgoViewNumber vEgoSleep))
		(Load RES_SOUND (GetSongNumber 24))
		(super init:)
		(StatusLine enable:)
		(StopEgo)
		...
		(cSound stop: number: (GetSongNumber 24) loop: -1 play:)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(ego view: (GetEgoViewNumber vEgoSleep) setLoop: 0 setCel: 0)
				(= seconds 3)
			)
			(1
				(TimedPrint 3 10 41)
...
(instance moveStoneAway of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(ego view: (GetEgoViewNumber vEgoMagicDetect) loop: 0 cel: 0 cycleSpeed: 1 setCycle: EndLoop)
				(= cycles 15)
			)
			(1
				(++ local0)
