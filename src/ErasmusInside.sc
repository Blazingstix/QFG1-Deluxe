...
(instance rm30 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vTeleportPink vWizardLobby vFenrus)
		(super init:)
	...
	(method (doit)
		...
		(cond 
			...
			((and (== (ego edgeHit?) SOUTH) (not leaving)) (= leaving TRUE) (= warpOut TRUE))
			(
		...
		(cond 
			...
			(
				(or
					...
					(MouseClaimed dragon event emRIGHT_BUTTON)
				)
				(HighPrint 30 8)
			...
			(
				(or
					...
					(MouseClaimed theLight event emRIGHT_BUTTON)
				)
				(HighPrint 30 9)
			...
			(
				(or
					...
					(MouseClaimed peacock event emRIGHT_BUTTON)
				)
				(if (> (peacock cel?) 0)
					(HighPrint 30 10)
			...
			(
				(or
					...
					(MouseClaimed theDunker event emRIGHT_BUTTON)
				)
				(HighPrint 30 12)
			...
			(
				(or
					...
					(MouseClaimed armor event emRIGHT_BUTTON)
				)
				(HighPrint 30 15)
			...
			(
				(or
					(Said 'look/airplane')
					(MouseClaimed plane event emRIGHT_BUTTON)
				)
				(HighPrint 30 16)
			...
			(
				(or
					...
					(MouseClaimed mummy event emRIGHT_BUTTON)
				)
				(HighPrint 30 17)
			...
			(
				(or
					(Said 'look/onklunk')
					(MouseClaimed lunk event emRIGHT_BUTTON)
				)
				(HighPrint 30 20)
			...
			(
				(or
					...
					(MouseClaimed theFrame event emRIGHT_BUTTON)
				)
			...
			(
				(or
					(Said 'look/chest')
					(MouseClaimed onChest event emRIGHT_BUTTON)
				)
				(HighPrint 30 24)
			...
			(
				(or
					...
					(MouseClaimed onBookcase event emRIGHT_BUTTON)
				)
				(HighPrint 30 26)
			...
			((MouseClaimed onWest event emRIGHT_BUTTON) (LookWestRoom))
			(
				(and
					(MouseClaimed onEast event emRIGHT_BUTTON)
					(not (MouseClaimed dragon event emRIGHT_BUTTON))
				)
				(LookEastRoom)
			)
			(
...
(instance warning of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(dragon setCel: RELEASE setCycle: Forward)
			...
			(1
				(= warned TRUE)
				(HandsOn)
				(dragon setCycle: 0 setCel: 0 stopUpd: setScript: NULL)
			)
		)
	)
)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(if (< (fenrus cel?) 2)
					(fenrus setCycle: CycleTo 2 cdFORWARD self)
				else
					(self cue:)
				)
			)
