...
(instance rm31 of Room
	...
	(method (init)
		...
		(LoadMany RES_SCRIPT 131 132 133 134 135)
		(LoadMany RES_TEXT 131 132 133 134 135)
		(LoadMany RES_VIEW vWizardTable vFenrus vTeleportPink)
		(StatusLine enable:)
		(super init:)
		(StopEgo)
		(ego init:)
	...
	(method (handleEvent pEvent)
		(switch (pEvent type?)
			(mouseDown
				(cond 
					((or erasmusTalking fenrusTalking) (pEvent claimed: 1))
					((MouseClaimed wizBody pEvent shiftDown) (LookErasmus))
					((MouseClaimed onRat pEvent shiftDown) (LookFenrus))
					((MouseClaimed onLWindow pEvent shiftDown) (LookWest))
					(
						(and
							(MouseClaimed onRWindow pEvent shiftDown)
							(not (MouseClaimed onCheese pEvent shiftDown))
						)
						(LookEast)
					)
					(
						(and
							(MouseClaimed onTable pEvent shiftDown)
							(not (MouseClaimed teacup pEvent shiftDown))
						)
						(LookTable)
					)
					(
					(and (MouseClaimed teacup pEvent shiftDown) (not erasmusDrinking)) (LookCup))
					((MouseClaimed onCheese pEvent shiftDown) (LookCheese))
					((MouseClaimed ego pEvent shiftDown) (LookHero))
					((MouseClaimed onCeiling pEvent shiftDown) (LookUp))
					((MouseClaimed onStairs pEvent shiftDown) (LookDown))
				)
			)
			(keyDown
				(cond 
				...
				(cond 
					...
					((Said 'ask>')
						(if erasmusDrinking
							...
							(if (and local263 (pEvent claimed?))
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(cond 
					((> (ego cel?) 1) (ego setCycle: CycleTo 1 cdBACKWARD self))
					((< (ego cel?) 1) (ego setCycle: CycleTo 1 cdFORWARD self))
					(else (self cue:))
				)
			)
			(1
			...
			(2
				(wizard setCycle: 0 setCel: 0 setScript: NULL)
...
(instance ratPrint of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(fenrus setCycle: 0 setLoop: 5 setCel: 0 setScript: NULL)
				(= fenrusTalking FALSE)
			)
		)
	)
		...
		(switch (= state newState)
			(0
				(switch local254
					(0
						(if (> (ego cel?) 0)
							(ego setCycle: CycleTo 0 cdBACKWARD self)
						else
							(self cue:)
						)
					)
					(1
						(if (> (ego cel?) 1)
							(ego setCycle: CycleTo 1 cdBACKWARD self)
						else
							(ego setCel: 1)
							(= cycles 1)
						)
					...
					(2
						(cond 
							((< (ego cel?) 2) (ego setCycle: CycleTo 2 cdFORWARD self))
							((> (ego cel?) 2) (ego setCycle: CycleTo 2 cdBACKWARD self))
							(else (self cue:))
						)
					)
					(3
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				...
				(wizard show:)
				(teacup show:)
				(wizDrinking setCel: 0 setScript: NULL)
			)
		)
	)
)
