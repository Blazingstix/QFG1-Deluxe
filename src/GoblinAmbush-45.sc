...
;	CI: This procedure wasn't auto-decompiled by the version of SCICompanion I used. A manual decompilation is above.
;(procedure (AimToward who what param3 &tmp distX distY)
;	;who = client, what = ego, param3 = [local66 register] (typically)
;	
;	(asm
;		pushi    #x
;		pushi    0
;		lap      who
;		send     4			;(who x?)
;		push    			;push (who x?) onto stack.
;
;		pushi    #x
;		pushi    0
;		lap      what		;(what x?)
;		send     4			;put (what x?) into accumulator
;
;		sub     			;accumulator = stack.pop - accumulator
;		sat      distX		;(= distX (who x?) - (what x?))
;
;		
;		pushi    2			;push 2 into immediate stack
;		pushi    #y
;		pushi    0
;		lap      who
;		send     4			;(who y?)
;		push    			;push (who y?) onto stack
;
;		pushi    #y
;		pushi    0
;		lap      what
;		send     4			;put (what y?) into accumulator
;
;		sub     			;acc = stack.pop - acc
;							;acc = (why y?) - (what y?)
;		mul     			;acc = acc * 2
;		sat      distY		;(= distY (* (- (who y?) (what y?)) 2)) ; distY = (y1 - y2) * 2
;
;		pushi    65426		;push -110 into stack/immediate
;		lat      distX		;load distX into acc
;		lt?     			;acc = pop() < acc
;							;(if (< -110 distX))
;		bnt      code_0040	;if not true, then make true, and if true return FALSE.
;		pprev   			
;		ldi      110		;accumulator = 110
;		lt?     			;(if (< distX 110))
;							;(if (and (< -110 distX) (< distX 100))
;
;code_0040:
;		not     
;		bt       code_0054		;goto return FALSE
;		
;		pushi    65426			;-110
;		lat      distY
;		lt?     				;(if (< -110 distY) ;aka $FF92
;		bnt      code_0050
;		pprev   				;push distY onto stack
;		ldi      110			;accumulator = 110
;		lt?     				;acc = pop < acc ==> (if (< distY 110)
;
;code_0050:
;		not     
;		bnt      code_0057
...
;
;code_0057:
;		lsp      param3		;push param3 onto stack
;		lst      distX		;push distX onto stack
;		lat      distX		;load distX into accumulator
;		mul     			;acc = pop*acc ==> distX * distX
;		push    			;push distX*distX onto stack
;		lst      distY		;push distY onto stack
;		lat      distY		;load distY into accumulator
;		mul     			;acc = pop*acc ==> distY * distY
;		add     			;acc = acc + pop ==> (distY * distY) + (distX * distX)
;		ult?    			;acc = pop < acc ==> param3 u< ((distY * distY) + (distX * distX))
;							;(u< param3 (+ (* distY distY) (* distX distX)))
;		bnt      code_006f	;not true, return false
;
;		ldi      FALSE
;		ret     			;(return FALSE)
;		jmp      code_0072	;end
...
(instance rm45 of Room
	...
	(method (init)
		...
		(LoadMany RES_VIEW vGoblinDefeated vGoblin)
		(StopEgo)
		(ego init:)
		(cSound stop:)
		(goblinS play:)
		(if (not numDeadGoblins)
			(Load RES_VIEW vGoblinBush)
		...
		(if (or (!= prevRoomNum GOBLIN) gMonsterHealth)
			(switch numDeadGoblins
				(0
	...
	(method (dispose)
		...
		(if (== prevRoomNum GOBLIN) (= monsterInRoom FALSE))
		(super dispose:)
	)
		...
		(cond 
			...
			(
				(and
					...
					(MouseClaimed goblin1 event emRIGHT_BUTTON)
				)
				(HighPrint 45 19)
				;It's R2D4!
			)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(== (event type?) mouseDown)
					(MouseClaimed goblin2 event emRIGHT_BUTTON)
				)
				(HighPrint 45 20)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(== (event type?) mouseDown)
					(MouseClaimed goblin3 event emRIGHT_BUTTON)
				)
				(HighPrint 45 21)
				;It's Droopy!
			)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(== (event type?) mouseDown)
					(MouseClaimed goblin4 event emRIGHT_BUTTON)
				)
				(HighPrint 45 22)
				;It's Sneaky!
			)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(== (event type?) mouseDown)
					(MouseClaimed goblin5 event emRIGHT_BUTTON)
				)
				(HighPrint 45 23)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(== (event type?) mouseDown)
					(MouseClaimed goblin6 event emRIGHT_BUTTON)
				)
				(HighPrint 45 24)
				;It's JohnnyFive!
			)
...
(instance movingBush of Actor
	...
	(method (handleEvent event)
		(if
			(and
				(== (event type?) mouseDown)
				(MouseClaimed movingBush event emRIGHT_BUTTON)
			)
			(HighPrint 45 25)
			;One strange bush!
		)
	...
	(method (handleEvent event)
		...
		(if (Said 'search/enemy,goblin,body,man,creature')
			(cond 
				...
				(= lootedGoblin1 TRUE) (GiveSilverCoins -5))
	...
	(method (handleEvent event)
		...
		(if (Said 'search/enemy,goblin,body,man,creature')
			(cond 
				...
				(= lootedGoblin2 TRUE) (GiveSilverCoins -4))
	...
	(method (handleEvent event)
		...
		(if (Said 'search/enemy,goblin,body,man,creature')
			(cond 
				...
				(= lootedGoblin5 TRUE) (GiveSilverCoins -8))
	...
	(method (handleEvent event)
		...
		(if (Said 'search/enemy,goblin,body,man,creature')
			(cond 
				...
				(= lootedGoblin6 TRUE) (GiveSilverCoins -4))
...
(instance goblinAttacks of Script
	(properties
		;register ;== goblinIndex. index of the goblin assigned to this script.
	)
