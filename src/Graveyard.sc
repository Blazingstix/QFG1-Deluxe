...
(procedure (LifeDrained)
	(EgoDead DIE_RETRY DIE_GRAVEYARD_GHOSTS 64 0
		#title { Oh-h-h-h-h-h-h!_} 
		#width 250
		#icon (GetEgoViewNumber vEgoDefeatedMagic) 0 0
...
(instance rm64 of Room
	(properties
		...
		north 52
		west 63
		;south is not specified, so as to prevent collision with the "falling down hole" script
	)
	
	(method (init)
		(LoadMany RES_SCRIPT GHOSTS CEMETERY)
		(LoadMany RES_VIEW vGhosts vMandrake (GetEgoViewNumber vEgoDefeatedMagic))
		(super init:)
		(StatusLine enable:)
		(cSound fade:)
		...
		(if
			(and
				...
				(< (+ dayPulledMandrakeRoot 1) currentDay)
			)
		...
		(StopEgo)
		(ego init:)
		(if (and isNightTime (Btst PROTECTED_UNGUENT)) (SolvePuzzle POINTS_USEUNDEADUNGUENT 2))
		(switch prevRoomNum
			(72
	...
	(method (doit)
		...
		(cond 
			((and
					...
					;(< (ego y?) 160)
					(not ghostsDrainEgo)
					(not (Btst PROTECTED_UNGUENT))
					(not (Btst GHOSTS_ATTACK))
					isNightTime
				)
				(Bset GHOSTS_ATTACK)
			)
		...
		(if (and isNightTime (< ghostCount 3))
			(switch local1
				(0
	...
	(method (dispose)
		...
		(if isNightTime
			...
			(if (not (Btst DIE_RETRY_INPROGRESS))
...
;CI:NOTE: This doesn't appear to be used anywhere
...
(instance riserGetsUp of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(riser setCel: RELEASE setCycle: EndLoop self)
			)
			(2
				(riser
					z: 35
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(if
					(or
						(and isNightTime (Btst VISITED_GRAVEYARD_NIGHTTIME))
						(and (not isNightTime) (Btst VISITED_GRAVEYARD_DAYTIME))
					)
					(client setScript: 0)
				else
					(= cycles 8)
...
(instance fallInHole of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(EgoDead DIE_RETRY DIE_GRAVEYARD_HOLE 64 36
					#icon (GetEgoViewNumber vEgoDeathScenes) 0 3 ;CI: moved views in Deluxe. Was 800 1 5.
		...
		(switch (= state newState)
			...
			(2
				(if (== timeZone TIME_MIDNIGHT)
					...
					(= dayPulledMandrakeRoot currentDay)
					;CI:TODO: Figure out some way around this little dead-end with pulling the mandrake root.
				)
