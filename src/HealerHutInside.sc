...
(instance rm55 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vHealer vHealerInside (GetEgoViewNumber vEgoFall) (GetEgoViewNumber vEgoBigGrin))
		(Load RES_SOUND 42)
		(super init:)
		...
		(StatusLine enable:)
		(StopEgo)
		(if (< colourCount 8) (healerWin color: 0 back: 15))
	...
	(method (handleEvent event &tmp spell)
		(cond 
			...
			((== mouseDown (event type?))
				(cond 
					((super handleEvent: event))
					((MouseClaimed healer event emRIGHT_BUTTON)
						(HighPrint 55 21)
			...
			((== evSAID (event type?))
				(cond 
					...
					((Said 'chat,ask>')
						...
						(if chatHealer (SolvePuzzle POINTS_TALKTOHEALER 2))
					)
					((Said 'gave,offer,sell,display,replace>')
						(cond 
							...
							((Said '/water[<floating]')
								(cond 
									((not (ego has: iFlyingWater)) (PrintDontHaveIt))
									(flyingWaterGiven
										(HighPrint 55 74)
...
(instance onKettle of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onKettle event emRIGHT_BUTTON)
				(HighPrint 55 14)
				;Whatever is boiling there is either soup or some sort of potion.
				)
		)
...
(instance onBird of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBird event emRIGHT_BUTTON)
				(HighPrint 55 95)
...
(instance onMaps of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onMaps event emRIGHT_BUTTON)
				(HighPrint 55 96)
				;Cheesecloth.
				)
		)
...
(instance onLitterBox of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onLitterBox event emRIGHT_BUTTON)
				(HighPrint 55 99)
...
(instance onThings of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onThings event emRIGHT_BUTTON)
				(HighPrint 55 102)
				;This and that.
				)
		)
...
(instance onStores of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onStores event emRIGHT_BUTTON)
				(HighPrint 55 103)
				;Ingredients.
				)
		)
...
(instance onBins of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBins event emRIGHT_BUTTON)
				(HighPrint 55 104)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				...
				(healer loop: 7 cel: 0 setCycle: CycleTo 4 cdFORWARD self)
			)
			(4
			...
			(7
				(ego view: (GetEgoViewNumber vEgo) setLoop: loopN setMotion: MoveTo 163 163)
				(healer
					setLoop: RELEASE
					cycleSpeed: 0
					moveSpeed: 0
			...
			(9
				(healer loop: 1)
				(ego view: (GetEgoViewNumber vEgoStanding) loop: loopE)
				(= cycles 2)
			)
			(10
				(HighPrint 55 113)
