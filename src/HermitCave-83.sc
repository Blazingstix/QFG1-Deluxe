...
(instance rm83 of Room
	...
	(method (init)
		(if (Btst DIE_RETRY_INPROGRESS)
			;CI: is there anything else we need to do to reset the death teleport??
			;I think let's just assume we've entered here for the first time, and let the game continue.
			(Bclr VISITED_HENRYINSIDE)
		...
		(StopEgo)
		(Load RES_SOUND 41)
		(LoadMany RES_SCRIPT 143 144 145)
				...
				(cond 
					...
					((Said 'ask>')
						...
						(cond 
							...
							(else (= chatHermit FALSE) (event claimed: TRUE) (self setScript: sElse))
						)
					...
					((Said 'gave,gave/ration')
						(cond 
							...
							(isNightTime
								(TalkObjMessages hermitTalk 83 12)
							...
							(else
								(TalkObjMessages hermitTalk 83 13)
								;"Me guest bed is the straw in the corner over there.
					...
					((Said 'sat/')
						(if (not sittingOnChair)
							(if (ego inRect: 140 150 174 174)
								(ego
									view: (GetEgoViewNumber vEgoSittingHenry)
									setLoop: 0
									ignoreActors: 1
									illegalBits: 0
									posn: 159 167
					...
					((Said 'cast>')
						;CI: moved the spell codes into a unified (Said cast>) block, like everywhere else in the game.
						...
						(switch spell
							...
							(else
								;CI:NOTE: if we want to bump this code back to the main script, uncomment this part here.
								;(event claimed: FALSE)
							)
						)
					)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(HandsOff)
				(TalkObjMessages hermitTalk 83 30)
	...
	(method (changeState newState)
		(if client
			(switch (= state newState)
				...
				(1
					(TalkObjMessages hermitTalk 83 33)
					;"Do you want it?"
				)
				(2
					(User canInput: TRUE)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(TalkObjMessages hermitTalk 83 34)
		...
		(switch (= state newState)
			(0
				...
				(if (Btst LEARNED_TRIGGER)
					(TalkObjMessages hermitTalk 83 35)
					;"You already 'ave me only scroll."
				else
					(TalkObjMessages hermitTalk 83 36)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(hermitTalk caller: self)
				(TalkObjMessages hermitTalk 83 37)
				;"Let's see.  'Ow does Erasmus do it?  Oh, yes."
			)
			(1
				(TalkObjMessages hermitTalk 83 38)
			...
			(3
				...
				(TalkObjMessages hermitTalk 83 39)
				;"There you 'as it."
			)
			(4 (HandsOn))
		)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(TalkObjMessages hermitTalk 83 40)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(hermitTalk caller: self)
				(HandsOff)
				(TalkObjMessages hermitTalk 83 41)
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				...
				(hermitTalk caller: self)
				(HandsOff)
				(TalkObjMessages hermitTalk 83 42)
...
(instance sGoSleep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(ego view: (GetEgoViewNumber vEgoThrowing) setLoop: 1 setCycle: EndLoop self)
			)
			(2
				(ego
					view: (GetEgoViewNumber vEgoFall)
					setLoop: 0
					setCel: 2
					posn: 72 168
					setPri: 13
	...
	(method (changeState newState)
		(switch (= state newState)
			(0
				(HandsOff)
				(hermitTalk caller: self)
				(TalkObjMessages hermitTalk 83 45)
