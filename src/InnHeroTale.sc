...
(instance rm301 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vInn vSheema vSheemaFood vInnCast)
		(LoadMany RES_SCRIPT 165 166 167 168 169 170)
		(LoadMany RES_SOUND 21 50)
		(rm301 style: (if (== prevRoomNum 302) 16 else 2))
		(super init:)
		...
		(if
			(and
				(or (== timeZone TIME_SUNSET) (== timeZone TIME_NIGHT))
				...
				(cond 
					...
					((Said 'eat')
						(switch foodOrdered
							...
							(mealATTABLE
								(cond 
									((Btst HERO_STARVING) (Bclr HERO_STARVING) (Bclr HERO_HUNGRY))
									((Btst HERO_HUNGRY) (Bclr HERO_HUNGRY) (= mealsPreEaten 1))
									(else (= mealsPreEaten 2))
								)
...
(instance shameen of Prop
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed shameen event emRIGHT_BUTTON)
					(HighPrint 301 6)
					;You see a small, cat-like creature known as a Katta.  You've heard that Kattas are common in the southern deserts.
				)
			)
				...
				(cond 
					...
					(
						...
						(cond 
							...
							((GiveSilverCoins roomPrice) (Bset RENTED_ROOMATINN) (shameen setScript: (ScriptID 167 0)))
							(else
								(HighPrint 301 47)
						...
						((Said 'buy,order,get>')
							(cond 
								...
								(Said '/food,breakfast,lamb,chicken,soup,plate,bowl')
									(cond 
										((not (GiveSilverCoins mealPrice)) (NotEnoughMoney))
										(
								...
								((Said '/drink,coffee,tea,cup,cup')
									(cond 
										((not (GiveSilverCoins drinkPrice)) (NotEnoughMoney))
										(
										(and (== teaOrdered foodOrdered) (== foodOrdered mealNOTHING))
											(HighPrint 301 90)
...
(instance onKitchenDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onKitchenDoor event emRIGHT_BUTTON)
				(HighPrint 301 95)
...
(instance onPlates of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(and
					(not (MouseClaimed onKitchenDoor event emRIGHT_BUTTON))
					(MouseClaimed onPlates event emRIGHT_BUTTON)
				)
				(HighPrint 301 96)
...
(instance onLargeTable of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onLargeTable event emRIGHT_BUTTON)
				(HighPrint 301 98)
...
(instance onSleepDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onSleepDoor event emRIGHT_BUTTON)
				(HighPrint 301 99)
