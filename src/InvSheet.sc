...
(procedure (DisplayMagicSpell spell x y &tmp spellIndex [str 8])
	...
	;and if it's the ZAP spell, *and* it's engaged, show a quick asterix
	(if (and (== spell ZAP) zapMeleeBonus)
		(Display
			206 12
			p_at (+ x 22) y
			p_mode teJustLeft
...
(class InvSheet of Object
	...
	(method (init &tmp i temp1 y fgColour totalWeight [str 40])
		...
		(= fgColour (if (< colourCount 16) vRED else vLRED))
		(Display
		...
		;Gold Coins
		(= fgColour (if (< colourCount 16) vGREY else vLGREY))
		(Display
