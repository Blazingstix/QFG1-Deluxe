...
(instance waterfall of Prop
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			(
			(or (Said '/cascade') (MouseClaimed self event emRIGHT_BUTTON))
			(HighPrint 87 1)
...
(instance rm87 of Room
	...
	(method (init)
		...
		(Load RES_VIEW vWater (GetEgoViewNumber vEgoWaterReflection)) ;CI: add vEgoWaterReflection
		(super init:)
		...
		(cond 
			...
			((!= detailLevel DETAIL_LOW) (geese setLoop: 6 init: setScript: flockScript))
		)
	)
...
(instance periScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(3
				(client setCel: RELEASE cycleSpeed: 3 setCycle: EndLoop self)
			)
			(4
				(ShakeScreen 4)
				(= seconds 2)
