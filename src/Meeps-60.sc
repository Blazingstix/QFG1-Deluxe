...
(local
	...
	furOnGround
	local100_2_2 =  25
	[local300_2_2 8] = [11 8 10 6 9 7 11 6]
	[local56_3_2_2 16] = [41 167 65 135 104 157 130 114 180 146 212 125 243 167 269 114]
...
(procedure (localproc_0466 &tmp temp0)
	(= temp0 0)
	(while (< temp0 8)
		(= [local2 temp0] (Clone aMeep))
		([local2 (= [local100_3_2 temp0] (Clone anEye))]
			setLoop: 5
			cel: 0
			setPri: (+ 1 [local300_2_2 temp0])
			posn:
				[local56_3_2_2 (* temp0 2)]
				(- [local56_3_2_2 (+ (* temp0 2) 1)] 4)
			init:
			stopUpd:
			setScript: [newAct_3 temp0] 0 (= [newAct_3 temp0] (Clone aMeepScript))
		)
		([local100_3_2 temp0]
			setLoop: 3
			cel: 0
...
(instance rm60 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vMeeps vGreenMeep (GetEgoViewNumber vEgoSwordSpirea) (GetEgoViewNumber vEgoThrowing) (GetEgoViewNumber vEgoPickupMeep))
		(LoadMany RES_SOUND
			(GetSongNumber 55)
			(GetSongNumber 56)
			(GetSongNumber 57)
		...
		(meepSound number: (GetSongNumber 55) init:)
		(babySound number: (GetSongNumber 74) init:)
		(thudSound number: (GetSongNumber 58) init:)
		(throwSound number: (GetSongNumber 31) init:)
		(localproc_0466)
		(if (< colourCount 8) (meepWin color: vBLACK back: vWHITE))
				...
				(cond 
						...
						(switch (= spell (SaidCast event))
							...
							(DAZZLE
								(if (CastSpell spell)
									(= meepsScared TRUE)
									(AnimateDazzle)
									(if bossOnScreen ([local2 3] setScript: bossMad))
								)
							)
							(FLAMEDART
								(if (CastSpell spell)
					...
					((Said 'chat>')
						...
						(cond 
							...
							(metBoss (event claimed: TRUE) ([local2 3] setScript: bossRises))
					...
					((Said 'ask>')
						...
						(cond 
							(
							(and newAct (not bossOnScreen) (Said '//meep<green')) (= chatBoss 0) ([local2 3] setScript: bossRises))
							((not bossOnScreen)
								(= chatBoss 0)
								(event claimed: TRUE)
								(cond 
									(metBoss ([local2 3] setScript: bossRises))
									((== meepsOnScreen 0) (event claimed: TRUE)
										(HighPrint 60 32)
										;You don't see any Meeps to talk to at the moment.
										)
							...
							((Said '//apple,apple,stuff,junk') ([local2 3] setScript: throwThings 0 0))
							((Said '//fur<green')
								(if furOnGround
									(BossSays 5 60 35)
...
(instance aMeepScript of Script
	...
	(method (changeState newState &tmp temp0)
		(switch (= state newState)
			...
			(1
				(cond 
					((and bossOnScreen (== detailLevel DETAIL_LOW)) (self changeState: 0))
			...
			(2
				...
				(meepSound
					number: (GetSongNumber (+ (Random 0 2) 55))
					play:
				)
			)
			(3
				([local100_3_2 register] setCycle: 0 stopUpd: hide:)
			...
			(4
				...
				(client
					posn: (client x?) (- (client y?) 4)
					setPri: (+ 1 [local300_2_2 register])
					setLoop: 5
					setCel: 0
					stopUpd:
				)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				...
				([newAct_3 newAct_2] changeState: 1)
				(-- maxMeeps)
				(= meepsScared 1)
				(= cycles 2)
			)
...
(instance hackMeep of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(1
				(ego view: (GetEgoViewNumber vEgoSwordSpirea) setLoop: 2 cel: 0 setCycle: EndLoop self)
			)
			(2
				(ego setLoop: 3 cel: 0 setCycle: EndLoop self)
				([newAct_3 newAct_2] changeState: 2 seconds: 0 cycles: 3)
			)
	...
	(method (changeState newState &tmp clientX clientY clientPriority)
		...
		(switch (= state newState)
			...
			(1
				...
				([local100_3_2 3] setCycle: 0 stopUpd: hide:)
				(client
					ignoreActors:
					view: vGreenMeep
					setLoop: 2
			...
			(4
				(thudSound number: (GetSongNumber 58) play:)
				(bossRock
					ignoreActors: 0
					setCycle: 0
					setCel: 0
...
(instance bossHides of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				([local2 3] setPri: [local300_2_2 3])
				(= seconds 3)
			)
			(2
				([local2 3]
		...
		(switch (= state newState)
			...
			(5
				...
				(client
					setLoop: 5
					cel: 0
					setPri: (+ 1 [local300_2_2 3])
					posn: [local56_3_2_2 6] (- [local56_3_2_2 7] 4)
					forceUpd:
					setScript: [newAct_3 3] 0 (= [newAct_3 3] (Clone aMeepScript))
				)
			)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				([local2 3] hide:)
				(ego
					view: (GetEgoViewNumber vEgoPickupMeep)
					setPri: 4
					setLoop: 0
					setCel: RELEASE
					cel: 0
			...
			(3
				(ego setCycle: CycleTo 1 cdBACKWARD self cycleSpeed: 5)
			)
			(4
			...
			(6
				...
				(ego posn: 130 103)
				([local2 3] show: view: vMeeps setLoop: 5 setCel: 0)
			)
			(7
				(StopEgo)
				(FaceObject ego [local2 3])
				([local2 3] setLoop: RELEASE setCel: RELEASE)
				(= meepsScared 0)
				(ego observeBlocks: bossBlock)
				(HandsOn)
				(= local300_3_2_2 0)
