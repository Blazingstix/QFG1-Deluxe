...
(instance rm67 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vBushes vBushEyes (GetEgoViewNumber vEgoThrowing))
		(super init:)
		...
		(if (and (Btst ENTERED_FOX_ROAD_1) (Btst ENTERED_FOX_ROAD_2) (not (Btst MET_FOX)))
			...
			(Bset MET_FOX)
			(Load RES_VIEW vFox)
			(Load RES_VIEW (GetEgoViewNumber vEgoDrawWeapon))
			(fox init: setScript: foxCallForHelp)
			(Load RES_VIEW (GetEgoViewNumber vEgoMagicFlameDart))
			(Load RES_VIEW (GetEgoViewNumber vEgoSwordSpirea))
			(HighPrint 67 0)
				...
				(cond 
						...
						(cond 
							...
							(
								...
								(cond 
									...
									((not (ego inRect: 55 111 120 120)) (event claimed: FALSE))
									(else (HighPrint 67 5)
							...
							((Said '/fox')
								;CI: TODO: check for the case where the fox hasn't shown up yet.
								(if foxOnScreen 
									(HighPrint 67 6) 
									;The fox seems to have a foot caught in a trap.
								else 
									(HighPrint 67 7)
									;The fox is gone.
								)
					...
					((Said 'cast>')
						(switch (= spell (SaidCast event))
							...
							(DAZZLE
								;CI: NOTE: we don't cast dazzle on the fox.
								(if foxOnScreen
									(HighPrint 67 13)
							...
							(CALM
								;CI: NOTE: we don't cast calm on the fox
								(if foxOnScreen
									(HighPrint 67 16)
							...
							(OPEN
								;CI: NOTE: we only cast open if the fox is onscreen.
								(if foxOnScreen
									(if (CastSpell spell)
...
(instance foxFreed of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				(fox setScript: NULL setCycle: Forward)
				(= seconds 2)
			)
			(8
				(CenterPrint 67 26)
			...
			(12
				...
				(ego view: (GetEgoViewNumber vEgoDrawWeapon) loop: 0 cel: 0 setCycle: EndLoop)
				(= cycles 35)
			)
			(13
				(StopEgo)
		...
		(switch (event type?)
			(evSAID
				(cond 
					...
					((Said 'throw/dagger,dagger') (AnimateThrowingDagger self))
					((Said 'throw/boulder') (AnimateThrowingRock self))
					(
