...
(instance rm77 of Room
	...
	(method (init)
		...
		(if
		(and (not isNightTime) (!= prevRoomNum 76) (Btst STAG_PRESENT))
			(Load RES_VIEW vStag)
		)
		(super init:)
		(StatusLine enable:)
		...
		(StopEgo)
		(cond 
			((and (not (Btst MET_DRYAD)) (not monsterInRoom))
				(Bset STAG_PRESENT)
				(= monsterInRoom NULL)
			)
			((and (Btst DRYAD_AGREED_HELP) (not monsterInRoom) (not (Btst STAG_PRESENT)))
				(switch (Random 0 1)
					(0 (Bclr STAG_PRESENT))
					(1 (Bset STAG_PRESENT))
				)
		...
		(switch prevRoomNum
			...
			(76
				;we were killed because we harmed the stag, so now we get to try it again.
				(if (Btst DIE_RETRY_INPROGRESS)
					(Bset STAG_PRESENT)
					(ego posn: 40 140 loop: loopE)
				else
	...
	(method (handleEvent event &tmp temp0)
		(switch (event type?)
			(evSAID
				(cond 
					((Said 'cast>')
						(switch (= spellCast (SaidCast event))
							(DETMAGIC
					...
					((Said 'throw/dagger,dagger')
						...
						(if (AnimateThrowingDagger temp0)
