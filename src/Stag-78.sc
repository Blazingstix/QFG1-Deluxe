...
(instance rm78 of Room
	...
	(method (init)
		...
		(cond 
			...
			((and (Btst DRYAD_AGREED_HELP) (not monsterInRoom))
				(switch (Random 0 1)
					(0 (Bclr STAG_PRESENT))
					(1 (Bset STAG_PRESENT))
				)
		...
		(switch (event type?)
			(evSAID
				(cond 
					...
					((Said 'throw/boulder')
						(= temp1 (if (cast contains: stag) stag else NULL))
						(if (and (AnimateThrowingRock temp1) (cast contains: stag))
							(Bset STAG_HURT)
							(FaceObject ego stag)
							(RedrawCast)
						)
...
(instance antwerped of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(EgoDead DIE_RETRY DIE_ANTWERP_FLATTENED 78 17
