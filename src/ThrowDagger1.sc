...
(procedure (AnimateThrowingDagger target &tmp temp0 temp1 daggerActor)
	(if (!= curRoomNum roomDaggersDropped)
	...
	(return
		(if (not (ego has: iDagger))
			...
			((= hitSound (Sound new:))
				number: (GetSongNumber 31)
				priority: 15
				init:
			)
			(= whoTarget target)
			((= daggerActor (Actor new:))
				view: (GetEgoViewNumber vEgoThrowDagger)
				setLoop: 2
				setCel: 0
				illegalBits: 0
				ignoreActors:
				...
				init:
				hide:
				setScript: knifeScript NULL target
			)
			(return TRUE)
		)
	)
		...
		(switch (= state newState)
			(0
				...
				(ego
					...
					cel: 0
					setCycle: CycleTo 9 cdFORWARD self
				)
				(++ daggersInRoom)
			)
			(1
			...
			(2
				...
				(if isHitTarget
					(-- daggersInRoom)
					(++ daggersInMonster)
					(hitSound stop: number: (GetSongNumber 29) play: self)
				else
					(= cycles 1)
				)
			)
