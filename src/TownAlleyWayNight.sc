...
(instance rm334 of Room
	...
	(method (init)
		(Load RES_VIEW vAlley)
		(Load RES_VIEW (GetEgoViewNumber vEgoDefeated))
		(super init:)
		...
		(if (not (Btst VISITED_ALLEY_NIGHT))
			(RedrawCast)
			(LookAround)
		)
	)
	...
	(method (handleEvent event)
		...
		(cond 
			...
			((and
					(== thievesAmbush TRUE)
					(or
						(MouseClaimed onDagger event emRIGHT_BUTTON)
						(Said 'look/dagger')
					)
				)
				(HighPrint 334 6)
			...
			((or
					(MouseClaimed coin event emRIGHT_BUTTON)
					(Said 'look/alm,item')
				)
				(if (<= (ego y?) 150)
					(HighPrint 334 7)
			...
			((and (== thievesAmbush TRUE) (MouseClaimed sneak event emRIGHT_BUTTON))
				(HighPrint 334 9)
				;He may be short, but the dagger makes him look 7 feet tall.
			)
			((and (== thievesAmbush TRUE) (MouseClaimed slink event emRIGHT_BUTTON))
				(HighPrint 334 10)
				;The florid face tells you that this thief is a regular at the "Aces and Eights" tavern.
			)
			((MouseClaimed onBricks event emRIGHT_BUTTON)
				(HighPrint 334 11)
				;The whole place is in need of repair.
			)
			((MouseClaimed onFirstBarrel event emRIGHT_BUTTON)
				(HighPrint 334 12)
				;Looks like this barrel once contained Dragon's Breath.  The bottom's eaten out.
			)
			((MouseClaimed onSecondBarrel event emRIGHT_BUTTON)
				(HighPrint 334 13)
			...
			((and (not (if (not thievesAmbush) (<= (ego y?) 132))) (Said 'climb'))
				(HighPrint 334 15)
...
(instance ambushScript of Script
	...
	(method (changeState newState)
		...
		(switch state
			...
			(3
				;(knife setCel: 2 addToPic:) ;CI: NOTE: removed addToPic
				(knife setCel: 2)
			...
			(9
				(EgoDead DIE_RETRY DIE_ALLEY 334 42
				...
				(Bclr SLINK_WARNING)
				(StopEgo)
				(ego posn: 120 187 loop: loopN)
				(client setScript: NULL)
				(sneak stopUpd: forceUpd: loop: 4 cel: 0)
				(slink stopUpd: forceUpd: loop: 3 cel: 0)
				(knife stopUpd: forceUpd: posn: 1000 1000 priority: 10)
			)
		)
	)
						...
						(cond 
							...
							(else
								...
								(if (not (ego has: iThiefLicense))
									...
									(HighPrint
										(Format @str 334 25 257 thievesGuildPassword)
										;"The password is '%s'."
									)
								)
