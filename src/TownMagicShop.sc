...
(procedure (BuySomething item spell price)
	(return
		(cond 
			...
			((and spell (ego knows: spell)) (PrintAlreadyDoneThat))
			((GiveSilverCoins price)
				(HighPrint 314 1)
...
(instance rm314 of Room
	...
	(method (init)
		(Load RES_VIEW vMagicShop)
		(Load RES_VIEW vZara)
		(LoadMany RES_SOUND 67 (GetSongNumber 45) (GetSongNumber 28))
		(super init:)
...
(instance zara of Prop
	...
	(method (init)
		...
		(theThunder number: (GetSongNumber 45) init: play:)
						...
						(cond 
							...
							((Said 'buy,get>')
								(cond 
									(
									(or (Said '/fetch') (Said '/spell,scroll<fetch'))
									(if (BuySomething 10 FETCH fetchPrice) (SolvePuzzle POINTS_LEARNFETCH 2 MAGE)))
									(
										(or
											(Said '/dart,flame,fire')
											(Said '/spell,scroll<fireball,dart,flame,fire')
										)
										(if (BuySomething 10 FLAMEDART flameDartPrice) (SolvePuzzle POINTS_LEARNFLAMEDART 2 MAGE))
									)
									(
									(or (Said '/open') (Said '/spell,scroll<open'))
									(if (BuySomething 10 OPEN openPrice) (SolvePuzzle POINTS_LEARNOPEN 2 MAGE)))
									((Said '/scroll,spell')
										(HighPrint 314 69)
										;"You may choose from the Fetch, Open, or Flame Dart Spells."
										)
									((Said '/potion<heal,healing') (BuySomething iHealingPotion 0 healingPrice))
									((Said '/potion<vigor,stamina') (BuySomething iStaminaPotion 0 vigorPrice))
									((Said '/potion<mana,magic,power') (BuySomething iManaPotion 0 manaPrice))
									((Said '/potion')
										(HighPrint 314 70)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(5
				...
				(theTeleport number: (GetSongNumber 28) init: play:)
			)
			(6
				...
				(zaraWarp setCycle: CycleTo 5 cdFORWARD self)
			)
			...
			(9
				(if (not isDemoGame) ;EO: What's with the demo check?
					(HighPrint 314 74)
...
(instance exitScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(zaraWarp setCycle: CycleTo 3 cdFORWARD self)
			)
			(2
				(client hide:)
...
(instance onFrog of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onFrog event emRIGHT_BUTTON)
				(HighPrint 314 76)
				;It's a frog.
				)
		)
...
(instance onPentagram of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onPentagram event emRIGHT_BUTTON)
				(HighPrint 314 77)
...
(instance onToaster of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onToaster event emRIGHT_BUTTON)
				(HighPrint 314 12)
				;A toaster oven, very rarely used (electricity not invented yet).
				)
		)
...
(instance onManta of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onManta event emRIGHT_BUTTON)
				(HighPrint 314 14)
				;Dried up Mantray (Manta jerky).
				)
		)
...
(instance onCrystalBalls of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onCrystalBalls event emRIGHT_BUTTON)
				(HighPrint 314 11)
				;Crystal balls, useful only to those initiated into the ways of witches.
				)
		)
...
(instance onRobe of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onRobe event emRIGHT_BUTTON)
				(HighPrint 314 10)
				;A hooded robe for when it rains in here.
				)
		)
...
(instance onTinsel1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onTinsel1 event emRIGHT_BUTTON)
					(MouseClaimed onTinsel2 event emRIGHT_BUTTON)
				)
				(HighPrint 314 9)
				;Tinsel and chaff, very good for avoiding radar.
			)
...
(instance onBottles of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBottles event emRIGHT_BUTTON)
				(HighPrint 314 8)
				;Bottles of mephitic potions.
				)
		)
...
(instance onBooks1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onBooks1 event emRIGHT_BUTTON)
					(MouseClaimed onBooks2 event emRIGHT_BUTTON)
					(MouseClaimed onBooks3 event emRIGHT_BUTTON)
				)
				(HighPrint 314 7)
				;Books dealing with sorcery, necromancy and other occult subjects.
			)
...
(instance onParphernalia of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onParphernalia event emRIGHT_BUTTON)
				(HighPrint 314 79)
...
(instance onAccoutrements of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onAccoutrements event emRIGHT_BUTTON)
				(HighPrint 314 80)
...
(instance onBat of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBat event emRIGHT_BUTTON)
				(HighPrint 314 81)
				;Darned if I know!
				)
		)
...
(instance onHead of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onHead event emRIGHT_BUTTON)
				(HighPrint 314 6)
				)
		)
	)
...
(instance onOdds1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onOdds1 event emRIGHT_BUTTON)
					(MouseClaimed onOdds2 event emRIGHT_BUTTON)
					(MouseClaimed onOdds3 event emRIGHT_BUTTON)
				)
				(switch (Random 1 5)
					(1
						(HighPrint 314 82)
