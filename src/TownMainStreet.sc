...
(instance rm310 of Room
	...
	(method (init)
		(if (and (>= timeZone TIME_SUNSET) [egoStats PICK])
			(Load RES_SOUND (GetSongNumber 35))
		)
		(Load RES_SOUND (GetSongNumber 46))
		(Load RES_SOUND (GetSongNumber 28))
		(Load RES_VIEW vTownMagicOutside)
		(Load RES_VIEW (GetEgoViewNumber vEgoTired))
		(super init:)
		...
		(StopEgo)
		(ego
			illegalBits: (if (> timeZone TIME_SUNSET) cWHITE else (| cWHITE cLCYAN))
			init:
		)
		(flameSound number: (GetSongNumber 46) init:)
	...
	(method (doit &tmp tmpEgoX tmpEgoY temp2 temp3)
		(super doit:)
		(cond 
		...
		(= temp3 eyeY)
		(cond 
			((<= tmpEgoY 125) (= temp3 54))
			((<= tmpEgoY 140) (= temp3 53))
			(else (= temp3 52))
		)
		(cond 
			((<= tmpEgoX 160) (= temp2 246) (= temp3 52))
			((<= tmpEgoX 200) (= temp2 248) (= temp3 52))
			((<= tmpEgoX 240) (= temp2 250))
			((<= tmpEgoX 270) (= temp2 252))
			((<= tmpEgoX 290) (= temp2 254))
			(else (= temp2 256))
		)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed magicDoor event emRIGHT_BUTTON)
			...
			(
				(or
					(MouseClaimed oldLadyDoor event emRIGHT_BUTTON)
			...
			(
				(or
					(MouseClaimed guildDoor event emRIGHT_BUTTON)
					(Said 'look/hall[<club]')
				)
				(if isNightTime
					(HighPrint 310 14)
			...
			(
				(or
					(and
						(not (MouseClaimed onMagicSign event emRIGHT_BUTTON))
						(MouseClaimed flames event emRIGHT_BUTTON)
					)
					(Said 'look/flame')
				)
	...
	(method (notify param1)
		(cond 
			...
			((not (TrySkill PICK tryPickLOL lockPickBonus))
				(HighPrint 310 1)
				;Picking locks looked a lot easier in the instruction book.
				)
			(
			(and (Btst VISITED_LOL_HOUSE) (< dayLOLBreakIn currentDay))
			(HighPrint 310 2)
			...
			(else
				...
				(oldLadyDoor facingLoop: (ego loop?) locked: 0)
			)
		)
	)
)
