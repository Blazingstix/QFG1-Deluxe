...
(instance rm320 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vTownDryGoodsOutside vDryGoodsInside (GetEgoViewNumber vEgoTired))
		(super init:)
		...
		(StopEgo)
		(ego init:)
		(if (and isNightTime [egoStats PICK])
			(Load RES_SOUND (GetSongNumber 35))
		)
		(if (< timeZone TIME_SUNSET) (Load RES_VIEW vCentaurGirl) (centaur init:))
		((= storeDoor (Door new:))
			view: vTownDryGoodsOutside
			loop: 0
			cel: 0
			...
			locked: (if isNightTime TRUE else FALSE)
			facingLoop: loopN
			entranceTo: 322
			init:
			setPri: 8
	...
	(method (doit)
		(cond 
			((> (ego y?) 188) (= exploringTown FALSE) (curRoom newRoom: 300))
			((< (ego x?) 1) (= exploringTown FALSE) (curRoom newRoom: 330))
			(
			(and (not (ego inRect: 210 130 319 185)) hildeAsks) (= hildeAsks 0))
		)
		(super doit:)
	)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(MouseClaimed onSheriffHouse event emRIGHT_BUTTON)
			...
			(
				(or
					(MouseClaimed onStore event emRIGHT_BUTTON)
			...
			(
				(or
					(MouseClaimed onStand event emRIGHT_BUTTON)
				...
				(cond 
					((>= timeZone 4)
						(cond 
							(
								(Said
									'/centaur,filly,shopkeeper,female,horse,creature,seller,daughter,girl'
								)
								(HighPrint 320 13)
	...
	(method (notify param1)
		(cond 
			...
			(
			(and (Btst VISITED_SHERIFF_HOUSE) (< daySheriffBreakIn currentDay))
			(HighPrint 320 3)
...
(instance centaur of Prop
	...
	(method (doit)
		(if
		(and (not (ego inRect: 210 130 319 185)) hildeAsks)
			(= hildeAsks 0)
		)
		(super doit:)
	)
		...
		(cond 
			...
			((Said 'ask>')
				...
				(cond 
					...
					((Said '//beet') ;EO: This should refer specifically to cherries, but here, "cherry" is a synonym for the other out-of-season produce.
						(HighPrint 320 30)
					...
					((Said '//*') (= chatHilde FALSE)
						(HighPrint 320 39)
						;"I am sorry.  I do not get about very much, and I do not really know about many things."
						)
				)
				(if chatHilde (SolvePuzzle POINTS_TALKTOCENTAURGIRL 1))
			)
			((Said 'buy,get>')
				(cond 
					((not (ego inRect: 210 130 319 185)) (event claimed: TRUE)
						(HighPrint 320 40)
						;If you want to buy something, go over to the vendor.
						)
					((Said '/carrot,food')
						(if (GiveSilverCoins 1)
							(HighPrint 320 41)
			...
			((MouseClaimed onCentaur event emRIGHT_BUTTON)
				(HighPrint 320 46)
				;The creature in charge of the Farmer's Mart is a rather lovely and shy-looking young Centaur.
				)
			((MouseClaimed onBarrel1 event emRIGHT_BUTTON)
				(HighPrint 320 47)
				;The barrel contains potent looking potatoes more suited to throwing than eating.
				)
			((MouseClaimed onBarrel2 event emRIGHT_BUTTON)
				(HighPrint 320 48)
				;The small apples give off a sweet but pungent odor.
				)
			((MouseClaimed onCarrots event emRIGHT_BUTTON)
				(HighPrint 320 49)
				;Carrots are good for night vision and lower one's cholesterol.
				)
			((MouseClaimed onTurnips event emRIGHT_BUTTON)
				(HighPrint 320 50)
				;The turnips seem to blush with pride.  You wonder if the farmer has been using Alar.
				)
			(
				(or
					(MouseClaimed onGreens1 event emRIGHT_BUTTON)
					(MouseClaimed onGreens2 event emRIGHT_BUTTON)
				)
				(HighPrint 320 51)
...
(instance centaurScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			(0 (= cycles 2))
			(1
				(if (not (Btst VISITED_VEGGIE_STAND))
			...
			(2
				(HighPrint 320 25)
				;The pretty young centaur looks at you and says, "Good day, and welcome to Spielburg!  Do you wish to buy some nice fresh fruits or vegetables?"
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(2
				(if sleepNearStall
					(HighPrint 320 54)
					;"I'm sorry, but I had to move you out of my stall!  It is very dangerous to sleep in the street at night.  I fear you have been robbed."
					)
			)
		)
	)
