...
(instance rm300 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vTownOutlook (GetEgoViewNumber vEgoTired) vOttoYoyo)
		(rm300
			style:
			(switch prevRoomNum
				(301 3)
		...
		(if (not (Btst VISITED_TOWNOUTLOOK))
			(= currentDay 0)
			(FixTime 11) ;start the game at 11am, on Day 0.
			(Bset fInMainGame)
		)
		(StatusLine enable:)
		(StopEgo)
		(ego
			illegalBits: (if (< timeZone TIME_SUNSET) (| cWHITE cYELLOW) else cWHITE)
			init:
		)
		(if isNightTime
		...
		(if (< timeZone TIME_SUNSET) (sheriff init:))
		((= innDoor (Door new:))
			view: vTownOutlook
			loop: 0
			posn: 71 122
	...
	(method (doit)
		...
		(cond 
			...
			(
				...
				(if (and (not (Btst VISITED_TOWNOUTLOOK)) (< timeZone TIME_SUNSET))
					(HighPrint 300 2)
		...
		(cond 
			...
			(
				(or
					...
					(MouseClaimed onJailSign event emRIGHT_BUTTON)
				)
				(HighPrint 300 5)
			...
			(
				(or
					(Said 'look/scissors')
					(MouseClaimed onScissors event emRIGHT_BUTTON)
				)
				(if (> (ego x?) 125)
					(HighPrint 300 6)
			...
			(
				(or
					(Said 'look/pole')
					(MouseClaimed onBarberPole event emRIGHT_BUTTON)
				)
				(if (> (ego x?) 125)
					(HighPrint 300 8)
			...
			(
				(or
					(Said 'look/hotel')
					(MouseClaimed innDoor event emRIGHT_BUTTON)
					(MouseClaimed onInnSign event emRIGHT_BUTTON)
				)
				(if (< (ego x?) 220)
			...
			(
				(or
					(Said 'look/gate')
					(MouseClaimed townGate event emRIGHT_BUTTON)
				)
				(HighPrint 300 11)
			...
			(
				(or
					...
					(MouseClaimed onBarberNote event emRIGHT_BUTTON)
				)
				(cond 
		...
		(switch (= state newState)
			...
			(1
				((= egoLookAround (View new:))
					view: (GetEgoViewNumber vEgoHeadTurn) ;was vTownOutlook
					loop: 0 ;was loop 8 in vTownOutlook
					cel: 0
					posn: 187 154
					setPri: 15
					ignoreActors:
...
(instance sheriff of Actor
	...
	(method (init)
		...
		(mouseDownHandler add: self)
		(super init:)
		(if (== detailLevel DETAIL_LOW) (self hide:))
	)
	...
	(method (handleEvent event)
		(cond 
			...
			(
				(or
					(Said 'look/meisterson,man')
					(MouseClaimed onSheriff event emRIGHT_BUTTON)
				)
				(HighPrint 300 38)
			...
			(
				(or
					(Said 'look/yo,yo')
					(MouseClaimed onYoyo event emRIGHT_BUTTON)
				)
				(HighPrint 300 39)
			...
			(
				(or
					(Said 'look/pipe')
					(MouseClaimed onPipe event emRIGHT_BUTTON)
				)
				(HighPrint 300 40)
