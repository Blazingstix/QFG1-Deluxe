...
(instance rm65 of Room
	...
	(method (init)
		(LoadMany RES_VIEW vBruno vTownGate)
		(super init:)
		...
		(if
			(and
				(or (== timeZone TIME_MIDDAY) (== timeZone TIME_MIDAFTERNOON))
				(Btst EXITED_TOWN_2)
				(or (not (Btst BEAR_GONE)) (Btst SPIED_THIEVES))
			)
			(Load RES_VIEW (GetEgoViewNumber vEgoDefeatedMagic))
		...
		(StopEgo)
		(ego init:)
		;block the entrance to the town, if it's nighttime
		(if isNightTime (ego illegalBits: (| cWHITE cLGREY)))
		(switch prevRoomNum
			(74
	...
	(method (handleEvent event)
		(cond 
			...
			((== (event type?) mouseDown)
				(if (MouseClaimed bruno event emRIGHT_BUTTON)
					(HighPrint 65 18)
...
(instance knifeEgo of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			;CI: NOTE: Restored delay before dying.
			(4
				;CI: NOTE: I've reduced the delay from 5 seconds to 1.
				(= seconds 1)
			)
			(5
				(if usedMagic
					(EgoDead DIE_RETRY DIE_BRUNO_OUTLOOK_MAGIC 65 65
					...
					(EgoDead DIE_RETRY DIE_BRUNO_OUTLOOK_DAGGER 65 66
						#icon (GetEgoViewNumber vEgoClimbing) 2 5
						#title {Your figure remains still and silent.}
					)
				...
				(= usedMagic FALSE)
				(= brunoWillKill FALSE)
				;reset ego's position based on how they entered the room
				(switch prevRoomNum
					(74 ;south
						(ego posn: 160 170 setLoop: loopN))
					(54 ;north
						(ego posn: 160 75 setLoop: loopS))
					(66 ;east
						(ego posn: 265 140 setLoop: loopW))
					(else ;the town (west)
						(ego posn: 30 135 setLoop: loopE))
				)
