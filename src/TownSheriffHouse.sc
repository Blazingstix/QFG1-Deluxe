...
(instance rm321 of Room
	...
	(method (init)
		(LoadMany RES_SCRIPT REVERSE 288 289 290)
		(LoadMany RES_TEXT 288 289 290)
		(LoadMany RES_VIEW vSheriffHouse vOttoAsleep vSheriffAsleep (GetEgoViewNumber vEgoBigGrin) (GetEgoViewNumber vEgoDeadBrigands) (GetEgoViewNumber vEgoDefeated))
		(LoadMany RES_SOUND 8 13
			(GetSongNumber 52)
			(GetSongNumber 14)
			(GetSongNumber 15)
			(GetSongNumber 35)
	...
	(method (doit)
		...
		(if (and (== (ego edgeHit?) SOUTH) (not (Btst OTTO_BACK_TO_BED)))
			(= daySheriffBreakIn currentDay)
			(curRoom newRoom: 320)
		)
		(if (not (Btst SHERIFF_AWAKENED))
			(cond 
				((and ;moving upStairs
						(> (ego x?) 35)
						(< (ego y?) 92)
					...
					(= egoUpstairs TRUE)
					(ego setScript: (ScriptID 288 1)) ;upStairs
				)
				((and ;moving downStairs
					...
					(= egoUpstairs FALSE)
					(ego setScript: (ScriptID 288 2)) ;downStairs
				)
			)
		)
		(super doit:)
				...
				(cond 
						...
						(cond 
							...
							((Said '/vase,bottle')
								(cond 
									((ego has: iVase)
										(event claimed: FALSE)
										;CI: I believe that after running through the room, the look command will
										;then look through your inventory (if it hasn't found any matches)
									)
					...
					((Said 'unlock,lockpick,crack/safe,hasp')
						(cond 
							...
							((== timesCrackedSafe 2)
								(ego setScript: (ScriptID 289 0) NULL timesCrackedSafe)
								;set the script to RM 289, Busted Script.
							)
							((TrySkill PICK 0 (- lockPickBonus 20))
								(miscMusic number: (GetSongNumber 35) loop: 1 play:)
								(HighPrint 321 50)
								;Ah, got it!
								(++ timesCrackedSafe)
								(safeDoor setCel: 1)
					...
					((Said 'lift,open/box,lid')
						(cond 
							...
							(else 
								;call the faceTheMusic script, from RM 289.
								;it's a cute little scene where the hero "almost" gets caught.
...
(instance onCandles of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onCandles event emRIGHT_BUTTON)
				(cond 
					((ego has: iCandelabra) (event claimed: FALSE))
...
(instance onVase of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onVase event emRIGHT_BUTTON)
				(cond 
...
(instance onMusicBox of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onMusicBox event emRIGHT_BUTTON)
				(cond 
...
(instance onPaintingDown of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onPaintingDown event emRIGHT_BUTTON)
				(if uncoveredSafe
					(event claimed: FALSE)
				else
					(BreakInPrint 321 19)
...
(instance onPaintingUp of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onPaintingUp event emRIGHT_BUTTON)
				(if uncoveredSafe
					(BreakInPrint 321 18)
					;Over the safe is a charming portrait of the Sheriff and his wife.
				else
...
(instance onSafe of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onSafe event emRIGHT_BUTTON)
				(cond 
...
(instance onPlant of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onPlant event emRIGHT_BUTTON)
				(HighPrint 321 65)
				;It's a potted plant.
				)
		)
...
(instance onLeftDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onLeftDoor event emRIGHT_BUTTON)
				(BreakInPrint 321 58)
				;There is some fairly loud snoring going on in this room. You hear: "Brckawwwww...zup-zup-zup-zup-zzzzz."
				)
		)
...
(instance onRightDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onRightDoor event emRIGHT_BUTTON)
				(BreakInPrint 321 59)
				;Very faintly, you hear snoring: "Prrrrrt...prrrrrt...prrrrrt."
				)
		)
...
(instance onBottomDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBottomDoor event emRIGHT_BUTTON)
				(BreakInPrint 321 61)
				;Faintly, you hear: "Nkaawww...prrrrrt... honk...zup-zup-zup-buh-buh-buh-phweep.
				)
		)
...
(instance onChair of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onChair event emRIGHT_BUTTON)
				(HighPrint 321 66)
...
(instance onCouch of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onCouch event emRIGHT_BUTTON)
				(HighPrint 321 67)
...
(instance onTable of RFeature
	...
	(method (handleEvent event)
		(cond 
			...
			((MouseClaimed onTable event emRIGHT_BUTTON)
				(HighPrint 321 68)
				;A solid looking table.
				)
		)
...
(instance onStuffedChair of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onStuffedChair event emRIGHT_BUTTON)
				(HighPrint 321 69)
