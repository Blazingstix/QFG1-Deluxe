...
;	egoOldSilver		;added by CI: used to restore money after death by Dragon's Breath.
;	egoOldGold			;added by CI: used to restore money after death by Dragon's Breath.
)
...
(procedure (NotEnoughMoney)
	...
	(crusher setScript: (ScriptID TAVERN_CRUSHER 0))
)
...
(instance rm331 of Room
	...
	(method (init)
		;(= egoOldSilver [invNum iSilver])
		;(= egoOldGold [invNum iGold])
		(Load RES_PIC 400)
		(LoadMany RES_VIEW vBarInside vCrusher vBartenderPouring vBartender (GetEgoViewNumber vEgoInsideBar))
		(LoadMany RES_SOUND (GetSongNumber 43) (GetSongNumber 44))
		(LoadMany RES_SCRIPT TAVERN_LOOK TAVERN_BREATH TAVERN_DRINK TAVERN_CRUSHER TAVERN_STOOL INPUTCOMP)
		(Load RES_TEXT 257)
		(super init:)
		...
		(= numberOfAlesDrunk 0)
		(head view: (GetEgoViewNumber vEgoInsideBar) init: stopUpd: hide:)
		(StopEgo)
		(ego init:)
		(addToPics
		...
		(dust2 init: setPri: 15 stopUpd: hide:)
		...
		(barSound number: (GetSongNumber 44) init:)
		(switch prevRoomNum
			(330
				(ego posn: 156 188 setMotion: MoveTo 156 175)
			)
			(332
				(ego posn: 120 150 loop: loopE)
			)
			(else 
	...
	(method (doit)
		...
		(cond 
			((and (ego inRect: 171 140 291 183) (not playingCards))
				(= playingCards TRUE)
				(ego setScript: cardScript)
			)
			((and (not (ego inRect: 171 140 291 183)) playingCards) 
				(= playingCards FALSE)
				(ego setScript: NULL)
			)
		)
	...
	(method (dispose)
		...
		(Bset VISITED_TAVERN_INSIDE)
		(super dispose:)
		(LoadMany FALSE TAVERN_BREATH TAVERN_DRINK TAVERN_CRUSHER TAVERN_STOOL TAVERN_LOOK)
	)
	...
	(method (handleEvent event)
		(cond 
				...
				(cond 
					...
					((ego inRect: 128 106 171 130)
						(if (Btst HERO_SITTING)
							...
							(ego setScript: (ScriptID TAVERN_STOOL 0))
						)
					)
					(nearBarstool
						(HighPrint 331 6)
				...
				(if (Btst HERO_SITTING)
					(cond 
						((and (== drinkInHand drinkNothing) (== drinkOrdered drinkNothing))
							(ego setScript: (ScriptID TAVERN_STOOL 1))
						)
						((and (== drinkInHand drinkNothing) (> drinkOrdered drinkNothing))
							(HighPrint 331 9)
			...
			((or (Said '[order,buy,get]/breath[<dragon]') (Said 'ask//breath[<dragon]'))
				(if bartenderAttention
					(cond 
						((not (Btst HERO_SITTING))
							(HighPrint 331 14)
						...
						(else
							...
							(if (GiveSilverCoins dragonBreathPrice)
								(HighPrint 331 22)
				...
				(cond 
					((Said '/ale')
						(switch drinkInHand
							...
							(drinkAle
								(++ numberOfAlesDrunk)
								(ego setScript: (ScriptID TAVERN_DRINK 0)) ;drinkDown
							)
							(else
								(HighPrint 331 31)
								;That ain't beer!
...
(instance bartender of Actor
	...
	(method (doit)
		(cond 
			((and (< (ego y?) 113) (not (Btst HERO_SITTING)) (not nearBarstool))
				(= nearBarstool TRUE)
			...
			((or nearBarstool (Btst HERO_SITTING))
				(= bartenderAttention TRUE)
			)
			((and (not nearBarstool) (not (Btst HERO_SITTING)))
	...
	(method (handleEvent event)
		(switch (event type?)
			(mouseDown
				(if (MouseClaimed bartender event emRIGHT_BUTTON)
					(HighPrint 331 39)
...
(instance crusher of Actor
	...
	(method (handleEvent event &tmp [password 10])
		(return
			(cond 
				((super handleEvent: event))
				...
				(
					(EventEqualsString
						event
						(Format @password 257 thievesGuildPassword)
					)
					(if (and (not [egoStats STEALTH]) (not [egoStats PICK])) ;this is to prevent those without thief skills from entering the guild
						...
						(self setScript: (ScriptID TAVERN_CRUSHER 0))
					else
						(self setScript: (ScriptID TAVERN_CRUSHER 1))
					)
				)
				((!= (event type?) evSAID))
				((Said 'display,made/sign[<thief]')
	...
	(method (doit)
		(cond 
			((> bartenderYCount 1)
				...
				(if bartenderBounce
					(bartender
						posn: (bartender x?) (- (bartender y?) 2)
					)
				else
					(bartender
						posn: (bartender x?) (+ (bartender y?) 2)
			...
			((== bartenderYCount 1)
				(= bartenderYCount 0)
				(self cue:)
				(= bartenderBounce (not bartenderBounce))
				;(if bartenderBounce 
				;	(= bartenderBounce FALSE)
				;else
				;	(= bartenderBounce TRUE)
				;)
			)
		)
		(super doit:)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(7
				...
				(if (Btst HERO_SITTING) (head setCel: 2))
				(= cycles 2)
			)
			(8
				(HighPrint 331 71)
			...
			(10
				...
				(= bartenderYCount 7) ;go down for 7 cycles (to get mug)
			)
			(11
				(bartender cel: 1)
				(= bartenderYCount 8) ;go up for eight (with mug).
			)
			(12
				(switch drinkOrdered
					(drinkAle (self cue:))
					...
					(drinkBreath
						(User canInput: TRUE)
						(DB setScript: (ScriptID TAVERN_BREATH 0)) ;breathScript
					)
				)
			)
			(13
...
(instance head of View
	(properties
		y 73
		x 150
		view vEgoInsideBar ;CI:NOTE: This gets initialized in the room Init to the proper Avatar view. 
		loop 2
		cel 3
	)
)

(instance dust2 of View
	(properties
		y 89
		x 149
		view vEgoInsideBar ;CI:TODO: Separate into a new view
		loop 2
		cel 4
		priority 15
	)
...
(instance onPaper of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onPaper event emRIGHT_BUTTON)
				(HighPrint 331 73)
...
(instance onButcher of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onButcher event emRIGHT_BUTTON) (if (Btst HERO_SITTING) (head setCel: 2))
				(HighPrint 331 74)
...
(instance onBaker of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onBaker event emRIGHT_BUTTON) (if (Btst HERO_SITTING) (head setCel: 2))
				(HighPrint 331 75)
...
(instance onGoon of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onGoon event emRIGHT_BUTTON)
				(HighPrint 331 76)
...
(instance onTableTop of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onTableTop event emRIGHT_BUTTON)
				(HighPrint 331 77)
...
(instance onTrapDoor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onTrapDoor event emRIGHT_BUTTON)
				(HighPrint 331 78)
...
(instance onTableBottom of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onTableBottom event emRIGHT_BUTTON)
				(HighPrint 331 79)
...
(instance onStool1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onStool1 event emRIGHT_BUTTON) (if (Btst HERO_SITTING) (head setCel: 1))
				(HighPrint 331 80)
...
(instance onStool2 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onStool2 event emRIGHT_BUTTON)
				(if (Btst HERO_SITTING)
					(HighPrint 331 81)
...
(instance onStool3 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onStool3 event emRIGHT_BUTTON)
				(HighPrint 331 82)
...
(instance onKeg1 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onKeg1 event emRIGHT_BUTTON)
				(HighPrint 331 83)
...
(instance onKeg2 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onKeg2 event emRIGHT_BUTTON)
				(HighPrint 331 84)
...
(instance onKeg3 of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onKeg3 event emRIGHT_BUTTON)
				(HighPrint 331 85)
...
(instance onFloor of RFeature
	...
	(method (handleEvent event)
		(cond 
			((super handleEvent: event))
			((MouseClaimed onFloor event emRIGHT_BUTTON)
				(HighPrint 331 86)
