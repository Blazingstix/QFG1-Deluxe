...
(procedure (WizGameCastSpell spell)
	(if spellCast (proc32_11))
	(= spellCast spell)
	(= currentTime_2 (GetTime 1))
	(= wizGameSpellTime (+ 2 (/ [egoStats spell] 10)))
...
(class Rock of View
	(properties
		...
		priority 0
		underBits 0
		signal (| stopUpdOn staticView) ;aka $0101
		nsTop 0
		nsLeft 0
		nsBottom 0
		nsRight 0
...
(instance wizGame of Room
	...
	(method (init &tmp temp0 temp1 temp2 theRockInfo theRockInfo_2)
		(LoadMany RES_SCRIPT 238 205 CURSORCOORDS 239 240 241 242)
		(LoadMany RES_VIEW vWizGameUI vWizGameMonster hpStatusView)
		(super init: &rest)
		(theGame setCursor: normalCursor TRUE)
		(++ magesMazePlayCount)
		(SolvePuzzle POINTS_PLAYWIZARDGAME 5 1)
		(= temp0 0)
		(while (< temp0 4)
			(repeat
	...
	(method (doit &tmp theCurrentTime_2 theMagesMazeCommand)
		...
		(if
			(and
				wizGameSpellTime
				(!= (= theCurrentTime_2 (GetTime 1)) currentTime_2)
			)
	...
	(method (dispose)
		...
		(Joystick 12 0)
		(super dispose:)
		(LoadMany FALSE 238 205 CURSORCOORDS 239 240 241 242)
	)
