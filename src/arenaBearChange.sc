...
(instance rmBear of Room
	...
	(method (init)
		(Load RES_PIC 400)
		(Load RES_PIC 14)
		(LoadMany RES_VIEW vBearHumanFight vBearFight vBearDefeated vArenaDazzle)
		(StatusLine enable:)
		(super init:)
		(HandsOff)
		(= monsterInRoom 0)
	...
	(method (doit)
		;CI:NOTE: These must be for debugging only... How sure how ego is supposed to hit the edge while in the Arena.
		(cond 
			((== (ego edgeHit?) EAST)
				;go onto the Kobold Cave
				(curRoom newRoom: 15)
			)
			((== (ego edgeHit?) SOUTH)
				;return to the Ogre Entrance.
				(curRoom newRoom: 13)
			)
		)
		(super doit:)
	...
	(method (dispose)
		(= gMonsterHealth (= monsterInRoom FALSE))
		(super dispose:)
	)
