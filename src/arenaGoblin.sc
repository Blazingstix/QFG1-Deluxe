...
(instance goblinArena of Arena
	...
	(method (init)
		(Load RES_VIEW vGoblinFight)
		(= monster goblin)
		(= monsterInRoom GOBLIN)
		(super init: &rest)
		(if (== prevRoomNum 45)
			(= goblinsInArena (+ numDeadGoblins 1))
		else
			(= goblinsInArena 1)
		)
		...
		(gobMusic number: (GetSongNumber 3) init: play:)
		(goblin
			view: vGoblinFight
			setLoop: 0
			cel: 0
...
(instance fightScript of Script
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(1
				(goblin action: 2 setCycle: CycleTo 2 cdFORWARD self)
			)
			(2
...
(instance goblinHurt of Script
	...
	(method (dispose)
		(goblinHead dispose:)
		(goblin setCel: RELEASE startUpd:)
		(super dispose:)
	)
		...
		(switch (= state newState)
			...
			(2
				...
				(switch (-- goblinsInArena)
					(0
						(if (== prevRoomNum 45) (++ numDeadGoblins))
						(= gMonsterHealth 0)
						(goblin dispose:)
