...
(instance flyRight of Script
	...
	(method (doit)
		(cond 
			((and (<= gMonsterHealth 0) (not (Btst FLAG_351)))
				(= gMonsterHealth 1)
			...
			((and (< (client y?) 15) (Btst MANTRAY_LEFT))
				(Bclr MANTRAY_LEFT)
			)
			((and (> (client y?) 80) (not (Btst MANTRAY_LEFT)))
				(Bset MANTRAY_LEFT)
			)
			(
				(and
...
(instance flyLeft of Script
	...
	(method (doit)
		(cond 
			((<= gMonsterHealth 0) (client setScript: killMantray))
			((<= (client distanceTo: ego) 52) ((ScriptID ENCOUNTER 0) cue:))
		...
		(switch (= state newState)
			(0
				...
				(client
					...
					setLoop: 6
					cycleSpeed: 1
					setPri: (if (< (ego y?) 100) 9 else RELEASE)
					setCycle: Forward
					setStep: 4 2
					illegalBits: 0
					stopUpd:
			...
			(7
				...
				(client
					setLoop: 2
					cel: 0
					setPri: (if (== curRoomNum 81) 9 else RELEASE)
					setStep: 4 1
