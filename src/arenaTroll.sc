...
(instance trollArena of Arena
	...
	(method (init)
		(= monster troll)
		(= monsterInRoom TROLL)
		(if (OneOf prevRoomNum 88 89)
			;it's in the Troll Cave, or the Brigand Troll Guard
			(= canEscape FALSE)
			;if we've been killed in an ambush by the Troll, then let's go back to the Antwerp room.
			(if (== prevRoomNum 89)
				(= prevRoomNum 84)
			)
		)
		(Load RES_VIEW vTrollFight)
		(super init: &rest)
		(trollLegs init:)
		(addToPics add: trollLegs doit:)
		(trollMusic number: (GetSongNumber 2) loop: -1 play:)
